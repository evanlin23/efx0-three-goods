...
Question (CS 580 course project, Fall 2026; answered yes, see Status): does every additive fair-division instance in which each agent positively values at most three goods admit a complete EFX₀ allocation (envy-free up to any good, where the removed good may be worthless to the envious agent)?

**Status** (details and evidence in [LEDGER.md](LEDGER.md)):
- **Proved, and machine-checked in Lean: conjecture D, and hence TARGET** ([proofs/lb_last_step.md](proofs/lb_last_step.md); Lean: `EFX.target` in [lean/EFX/Target.lean](lean/EFX/Target.lean), `EFX.LB.corollaryD` in [lean/EFX/CorollaryD.lean](lean/EFX/CorollaryD.lean), see [lean/README.md](lean/README.md)). Every additive instance in which every agent positively values at most three goods has a complete EFX₀ allocation (Lean: natural-number values, hence rational by scaling; real values by a standard sign-pattern reduction, written, not formalized; see [lean/README.md](lean/README.md), "Not formalized"). The proof uses construction LB⁺: serial dictatorship with LB's junk placement, plus one "rotation" along a chain of agents when the natural owner of the large bundle fails. Two independent reviews of the written proof found no error. The items below are the partial results that came before.
