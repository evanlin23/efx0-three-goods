...
- **New, pending independent review: TARGET and conjecture D proved** ([proofs/lb_last_step.md](proofs/lb_last_step.md)). Construction LB⁺ is serial dictatorship with LB's junk placement, plus one "rotation" along a chain of agents when the natural owner of the large bundle fails. It always returns an EFX₀ allocation with at most one bundle of more than two goods, whenever every agent values exactly three goods and is balanced. With the reduction to cores, every instance with at most three relevant goods per agent has an EFX₀ allocation. The items below are the state before this proof.
