...
- **A written proof of conjecture D, and hence of TARGET, exists, and is being formalized in Lean** ([proofs/lb_last_step.md](proofs/lb_last_step.md); formal/lbplus). It uses construction LB⁺: serial dictatorship with LB's junk placement, plus one "rotation" along a chain of agents when the natural owner of the large bundle fails. Two independent reviews found no error. The ledger does not claim D or TARGET until the formalization lands. The items below are the claimed state.
