...
allocation is EFX₀ by the raw definition, and it has the D2 shape. All 5,400 records pass. Corrupted records were
tried by hand: a shifted allocation, a value vector outside the domains, and an allocation giving every good to one
agent. Each was reported.
...
The random-profile logs name commit bd5cb4d. They were run with the code of commit 78a81fa before it was committed.
That commit adds only docstrings, the commit printer, and `d_stress_check.py`'s file mode and self-test change; the
decision code is the same. The check logs name 78a81fa, or af85fd1 (the second encoding solved by CaDiCaL).
...
- D2 was satisfiable in every self-test profile (the streams replayed with `d_stress.py`, last line of the log), so the D2-unsatisfiable direction has not been cross-checked on a
  real instance.
- The unsatisfiable agreement is on "all bundles ≤ 2": trivial on the chains (m > 2n) and non-trivial in 4 cases.
- For m > 2n "all bundles ≤ 2" is unsatisfiable by counting, and its SAT calls are pigeonhole-hard. They ran on H_1
  and H_2 but are skipped on H_3 and tree 3, which compare D2 only.
...
one encoding only. The reviewer confirmed H_1's case (agent 4 = y_1 cannot own) by brute force.
...
- **chain t:** H_t of `k4/c4.md` §7. A head ℓ = {g_1, z, u, u′}, then t gadgets. Gadget j has three x's
  ...
  n = 4t + 1, m = 10t + 3. `chain(t)` is H_t up to a labelled isomorphism (the reviewer checked t ≤ 7).
...
- **pure n m:** one random connected pure core per (n, m), fixed by its seed and printed in the log. Every agent is
  4-good, m is small, so β = 3n − m + 1 is high (11 to 21). Since m < 2n, the large bundle is not forced there. The
  reviewer found an allocation with *all* bundles ≤ 2 in every sample taken, so these profiles do not exercise the
  large bundle.
...
- **H_1:** 3 climbs of 60 steps, minimum **4 of 5** feasible owners. This is ≥ 4 by SAT witnesses, and = 4 by one
  encoding.
- **H_2:** 4 climbs of 20–30 steps (two of them logged as "cycle 2", which was H_2 then), **9 of 9**. The traces are
  flat.
- **Pure cores (n = 6–10):** the count stayed at n, but trivially. With m < 2n an allocation with every bundle ≤ 2
  exists, so every agent is a feasible owner by construction.
