/* lb4r_tau: LB4r on one profile under many random insertion sequences (the exists-tau question of k4/lb4.md §5).
   Derived from k4/c4_lb4w.c of branch proof/k4-c4 (at b91b013; k4/lb4.c with 64-bit good masks,
   up to 32 agents, a direct owner search), which is used unchanged except for:
   -i10 -TN: every agent must have a single type (one profile). For each of N samples, the insertion sequence is drawn
   by a uniformly random choice among the candidates at every insertion step of Phase 1 (seeded, -XS sets the seed),
   and LB4r (-u3: every policy) is run with rotation bounds 0, 1, ..., R (-rR) in turn, so the first success gives the
   fewest rotations for that sequence over all policies (as lb4.c -d2). Output: one line per sample, then a histogram
   of that number (or 'fail'), and on stderr one example sequence per value. Every success is checked against the raw EFX0 definition. */
/* c4_lb4w: k4/lb4.c with 64-bit good masks (m <= 64), up to 32 agents, and the owner search enumerating the sets C
...
static long TAU = 0; static uint64_t rng_x = 88172645463325252ull;     /* -TN samples, -XS seed */
...
static void phase1(void) {
    ...
    for (int step = 0; step < n; step++) {
        ...
        if (best < 0) {              /* insertion step: every unprocessed agent has all its goods */
            ...
            else if (INS >= 1) { if (nins >= nchoice) choice[nchoice++] = INS == 10 ? (int)(rnd() % (uint64_t)nc) : 0; c = choice[nins]; maxchoice[nins] = nc; if (c >= nc) c = nc - 1; }
            nins++;
            best = cand[c]; b++;
        }
        int i = best; Y[i] = -1;
...
static void report(const char *what) {
    ...
    fprintf(stderr, " upg=");
    for (int i = 0; i < n; i++) if (upg[i]) fprintf(stderr, "%d:%lx ", i, (unsigned long)base[i]);
    ...
    while (scanf("%d %d", &n, &m) == 2) {
        ...
        if (TAU > 0) {                   /* -i10 -TN: random insertion sequences on one profile */
            ...
            for (int i = 0; i < n; i++) {
                ...
                cp[i] = 0; ts[i] = 1;
                for (int r = 0; r < d[i]; r++) ord[i][r] = gl[i][pr[i][0][r]];
                for (int g = 0; g < m; g++) rp[i][g] = -1;
                for (int r = 0; r < d[i]; r++) rp[i][ord[i][r]] = r;
            }
            for (long smp = 0; smp < TAU; smp++) {
                int k = RMAX + 1, ok = 0;
                nchoice = 0;             /* the first run draws the sequence; later bounds reuse it */
                for (int cap = 0; cap <= RMAX; cap++) {
                    ROT = cap;
                    if (run_leaf(&ok)) { fprintf(stderr, "split with a single type per agent\n"); return 1; }
                    if (ok) { if (!rawcheck()) { report("RAWFAIL"); return 2; } k = cap; break; }
                }
                hist[k]++;
                if (k > RMAX) printf("sample %ld: fail\n", smp); else printf("sample %ld: rot %d\n", smp, k);
                fflush(stdout);          /* one line per sample, so a run cut short keeps its results */
                if (!shownr[k]) { char lab[40]; snprintf(lab, sizeof lab, k > RMAX ? "TAU fail" : "TAU rot=%d", k); report(lab); shownr[k] = 1; }
            }
            ROT = RMAX;
            printf("tau samples %ld:", TAU);
            for (int k = 0; k <= RMAX; k++) printf(" rot%d %ld", k, hist[k]);
            printf(" fail %ld\n", hist[RMAX + 1]); fflush(stdout);
            continue;
        }
