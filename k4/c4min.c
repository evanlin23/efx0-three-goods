...
static int nex = 0, xcheck = 0, dist = 0, onlyf = -1;
...
static const char *fname[NF] = {"robF", "robfree", "-rhoF", "-poolexp", "safeall", "nvalid", "-poolthr1", "-rhoL",
  "-thrL", "rob", "-exposedL1", "toppairs", "-rhoLQ", "-poolvalF", "robfree+robF", "-maxrhoLQ"};
typedef struct { mask_t H[MAXN]; mask_t L; int comp; int F[NF]; } cfg_t;
...
int main(int argc, char **argv) {
  ...
  printf("RESULT profiles %lld omega>=1 %lld configs %lld none %lld keyfail %lld xbad %lld\n", nprof, nom, ncfg, ncomp_none, nkeyfail, nxbad);
  for (int p = 0; p < nphi; p++) {
