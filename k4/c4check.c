...
enum { C_W1, C_A1VIOL, C_E4, C_E4E_ROK, C_E4E_RFAIL, C_A3VIOL, C_BADVIOL, C_ROTINV, C_E4AFTER_OK, C_E4AFTER_FAIL,
       C_B_OK, C_B_VIOL, C_E4F, C_E4T, C_NCHK };
...
static void check_AB(int S) {
    ...
    if (e4) { chkf[C_E4] = 1; chkf[e4f ? C_E4F : C_E4T] = 1; return; }
    if (rok) { chkf[C_E4E_ROK] = 1; return; }
    chkf[C_E4E_RFAIL] = 1;
    for (int x = 0; x < n; x++) if (E[x] && (d[x] != 3 || Y[x] != ord[x][0] || !is_leader(x))) { chkf[C_A3VIOL] = 1; report("A3VIOL"); return; }
    int ks = -1;
    ...
    if (!bad) { chkf[C_BADVIOL] = 1; report("BADVIOL"); return; }
    ...
    if (!valid) { chkf[C_ROTINV] = 1; report("ROTINV"); }
        ...
        if (e42) chkf[ok ? C_E4AFTER_OK : C_E4AFTER_FAIL] = 1;
        else if (ok) chkf[C_B_OK] = 1;
        else { chkf[C_B_VIOL] = 1; report("B4VIOL"); }
    }
