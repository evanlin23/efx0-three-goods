/* c4min_f1.c: exhaustive checks of k4/c4min_f1.md (C4min with one frozen agent), written from the definitions of
   ...
   Theorem checks (counters in the RESULT line):
     f1fail    (r, Lambda)-maxima whose frozen agent is not big-top (or is big-top with omega = 1) and that have no
               owner valid with C = {}                                                   (Thm F1, Lemma 8(b))
     ...
     cov3 / cov4  profiles with a Psi-maximum whose frozen agent has three goods / four goods, not big-top (and no
               such maximum with three goods): Theorem F1 applies; btomega1: every Psi-maximum has a big-top frozen
...
static int *cfr, *cfl, *cft, *cfc0, *cfc, *cfbt, *cfp; static int capfeat;
...
static void do_profile(void) {
  ...
  for (int q = 0; q < nkeys; q++) {
    ...
    if (ncf > capfeat) { capfeat = ncf + 4096; cfr = realloc(cfr, capfeat * sizeof(int)); cfl = realloc(cfl, capfeat * sizeof(int)); cft = realloc(cft, capfeat * sizeof(int)); cfc0 = realloc(cfc0, capfeat * sizeof(int)); cfc = realloc(cfc, capfeat * sizeof(int)); cfbt = realloc(cfbt, capfeat * sizeof(int)); cfp = realloc(cfp, capfeat * sizeof(int)); }
    for (int a = before; a < ncf; a++) {
      ...
      psi(c, &cfr[a], &cfl[a]); cft[a] = tthr(c); cfbt[a] = isbt[c->xf]; cfp[a] = pc(c->L & U[c->xf]);
  ...
  for (int a = 0; a < ncf; a++) {
    ...
    if ((!cfbt[a] || omega == 1) && !cfc0[a]) { f1fail++; if (exf1 < nex) { exf1++; show("Theorem F1 fails", &cf[a]); } }
  }
  if (has3) cov3++; else if (has4) cov4++; else if (omega == 1) covbt1++; else { covbt++; if (anycomp) covbtc++; }
  if (!anycomp) { starfail++; if (exstar < nex) { exstar++; printf("EX Theorem F1* fails:"); print_prof(); printf("\n"); } }
  { /* Conjecture Phi' = (-t, r, Lambda, -p) of k4/c4min.md §4: every maximum completable? */
    int b0 = -99, b1 = -1, b2 = -1, b3 = -99, bad = 0;
    for (int a = 0; a < ncf; a++) {
      int k0 = -cft[a], k3 = -cfp[a];
