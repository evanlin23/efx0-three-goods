...
- **No rule for the key that looks only at x works** (`attempts/k4_c4min_reduce_rules.py`, `results/k4_red_rules.log`;
  6,000 random profiles per n = 3 core, 8,402 with f = 1). Pick the keys whose x maximizes a score, then require every
  (r′, Λ′)-maximum there to be completable. The number of profiles on which each rule fails:

  | score of x | failures |
  |---|---|
  | level of its top | 241 |
  | fewest private goods | 239 |
  | a / v(U_x) | 215 |
  | a / v(U_x) minus the private share | 157 |
  | least value of private goods | 145 |
  | best (r′, Λ′)-maximum of I′ | 120 |
...
- **Even the best key fails with Theorem Z's potential.** Instance A2, the core H★ (every agent has top 7, is of type
  ...
  128 profiles with n ≤ 3 have no key at which every (r′, Λ′)-maximum is completable, all in this core. As the maxima
...
Replay: `python3 attempts/k4_c4min_reduce_attempts.py` (log `results/k4_red_attempts.log`, "ALL CONFIRMED").
