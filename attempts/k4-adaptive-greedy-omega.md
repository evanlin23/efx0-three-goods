...
1 = {2, 3, 4, 5}, 2 = {2, 3, 4, 5} with values (1, 4, 6, 8), (2, 3, 4, 8), (2, 7, 8, 4). Every first agent gives ω = 2
after upgrades of any policy, so the rules insert agent 0 first (index order on the tie); that run needs two nested
rotations under every policy, while inserting agent 1 first needs one (rule F). Brute force: 5 EFX₀ allocations, all
