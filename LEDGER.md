...
| R1 | EFX₀ exists for every instance with ≤ 3 relevant goods per agent and n ≤ 6 | CERTIFIED | `results/certs_5_6.json.gz`, `results/frontier_results_5_6.json` | conditional on Mahara (m ≤ n + 3); 251 connected cores; CI re-checks without SAT |
| R2 | Conjecture D holds for n = 7, m = 13 | CERTIFIED | `results/frontier_results_7.json` | 37 cores; certificate file not saved yet: regenerate with `src/run7.py 13` |
...
## Lessons
- About 3,000 random cores "supported" conjecture A; exhaustive search refuted it in minutes (failures: 2 to 36 of 46,656 profiles per hypergraph).
- Every UNSAT claim that refutes a conjecture gets a second, independently written encoding before it enters this ledger.

## Open items
1. Independent re-implementation of the core enumeration (the trust point of R1): reproduce 15 / 211 / 25 / 37.
2. Verify the Mahara citation (m ≤ n + 3).
3. Regenerate the n = 7, m = 13 certificates; run m = 12 and 11 (`src/run7.py`); then n = 8 at m = 15.
