...
**Beyond n = 7 (exhaustive, not certified).** LB never fails on any connected core with n = 8 and m ∈ {15, 16} (53 cores, 89,019,648 pairs; `results/construct_8.log`). Every output is checked by the raw check in `construct.c`, but no certificate file is stored.
...
**Open.**
1. S2.LB: LB never fails. Theorem 1 reduces this to the existence of an owner in the last step. Certified for every core with n ≤ 6 (S2.N6) and every connected core with n = 7 (S2.N7). For n = 8, see `results/construct_8.log` (exhaustive, raw check in `construct.c`, no stored certificate).
