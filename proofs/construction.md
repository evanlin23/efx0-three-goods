...
**n = 7 (certified; ledger S2.N7).** LB never fails on any connected core with n = 7, at every m (41,170 cores, 11,524,965,120 core–profile pairs; `results/construct_7_cert.log`, workstream compute/lb-hunt). Every output is checked by the raw check in `construct.c`, and the certificates `results/certs_lb_7_{3_7,8,9,10,11_14}.json.gz` (LB's own outputs, split by m) are accepted by `tools/check_certs.py --require-d` and complete by orbit counting (`results/check_certs_lb_7.log`). This includes the 37,488 cores with m ≤ 10 that R5 had left to Mahara's theorem. LB's non-failure is construct.c's exhaustive count (raw EFX₀ check in the same program); C and Python agree on the sampled cores in `results/construct_7_python_sample.log`. The certificates independently confirm D, not LB's non-failure. LB uses a large bundle only at m ≥ 9. The earlier runs at m = 10–14 (`results/construct_7.log`) stored no certificate.

**n = 8, m ≥ 13 (certified; ledger S2.N8).** LB never fails on any connected core with n = 8 and m ∈ {13, …, 16} (12,763 cores, 21,436,939,008 core–profile pairs; `results/construct_8_13.log`, `results/construct_8_14.log`, `results/construct_8_15_16.log`). The certificates (m = 13 in four parts) are accepted by `tools/check_certs.py --require-d` and complete by orbit counting (`results/check_certs_lb_8.log`). LB's non-failure is construct.c's exhaustive count (raw EFX₀ check in the same program); C and Python agree on the sampled cores in `results/construct_8_python_sample.log`. The certificates independently confirm D, not LB's non-failure. n = 8, m ≤ 12 was not run. The earlier run at m = 15, 16 (`results/construct_8.log`) stored no certificate.

Note that LB breaks ties by index, so it is not invariant under relabelling. The runs above (Theorem 3, n = 7, n = 8) test one labelling per isomorphism class, the one `cores_nauty.py` produces (stored in the certificates). R3 and D are invariant, so their certification does not depend on this; "LB never fails" holds for this labelling only.
...
**Open.**
1. S2.LB: LB never fails. Theorem 1 reduces this to the existence of an owner in the last step. Certified, in the labelling stored in the certificates (produced by `cores_nauty.py`), for every core with n ≤ 6 (S2.N6), every connected core with n = 7 (S2.N7), and every connected core with n = 8, m ≥ 13 (S2.N8).
