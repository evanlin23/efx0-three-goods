...
- **q** (Kaviani et al. 2407.05139, Def. 2.2, p. 3): the largest number of common neighbours of two agents in B(I), i.e. two agents share at most q relevant goods. CFKS §6 phrases q as an edge *multiplicity*; Sgouritsa–Sotiriou §1.3 (p. 4) note that Kaviani et al. changed the notation to this pair-codegree form.
...
2. **(p, q)-bounds do not bound k.** Take a star: one agent shares one good with each of many others. It is (2, 1)-bounded and its centre has unbounded degree. The two families are incomparable, and the interesting classes are intersections. (2, ∞) ∩ {k ≤ d} is the multigraphs of maximum degree d (private goods as pendant edges); (∞, 1) ∩ {k ≤ d} is the linear hypergraphs of agent degree ≤ d, up to how goods valued by a single agent are embedded: several such goods of one agent become repeated singleton hyperedges, a multi-hypergraph.
3. **Hypergraph language.** The papers below put agents at vertices and goods on (hyper)edges, so a good valued by p agents is a hyperedge of size p. There:
   - q = 1 is "girth at least 3". Kakatelis et al. 2606.26948 (earlier) say girth ≥ 3 means "any two agents share at most one good/edge" (p. 2); Lianeas et al. 2608.03171 (earlier) say the same (Obs. 2.1, p. 4). The match is exact only up to the embedding of single-valuer goods: an agent's several private goods are repeated singleton hyperedges, making a multi-hypergraph. Kakatelis et al.'s Observation 2 (p. 6) allows "one good and its repetitions".
...
- **Techniques.**
  ...
  - Theorem 8.1 adds rules that finalize agents one at a time. It uses the p = 2, restricted-additive structure. Every vertex of G₀ has in-degree exactly 1, and when no earlier rule applies G₀ is a union of disjoint cycles (Lemma 8.6, p. 25).
...
- **Techniques.**
  ...
  - Rules 1–5 (pp. 8–15) finalize agents. Two-cycles (Rule 3) are handled in two ways:
    - when its two agents are the last ones left, with Mahara's two-valuation theorem: as restated in Theorem 4.4 (p. 10), a partial EFX allocation for agents with at most two distinct valuations extends to all goods without lowering anyone's utility (Mahara, *Discrete Appl. Math.* 340 (2023); arXiv 2008.08798, abstract read);
    - otherwise, with a choice protocol among three bundles (pp. 6 and 10).
...
- **Technique.** The 3PA algorithm, built on critical goods (worth more than half of one's bundle, Def. 2.7). On multigraphs (MultigraphAllocate, p. 20), the goods critical for two agents are given to a source of the enhanced envy graph. The remaining critical goods are handled by UncontestedCritical, and then Envy Cycle Elimination finishes.
...
- **Techniques: three steps.**
  1. An initial EFX orientation with few or well-placed envied vertices. For Theorem 2 this is a maximum-weight matching between agents and their first- and second-choice goods: single edges, weights 1 and 0 (§3, Lemma 3.7). It leaves at most ⌊n/2⌋ envied agents. With many parallel edges the matching is over bundles instead (§4, Def. 4.8, Lemma 4.9).
...
- **p = 2, exact.**
  - Afshinmehr et al. 2606.18665 (EC 2026; earlier, full): exact EFX₀ on all multigraphs, cancelable valuations. This is the theorem behind T3 and K4.MC7, and for additive valuations it subsumes 2.1's Theorem 8.1, 2.2's Theorem 4.9 and 2.4's Theorem 4.4. It does not subsume 2.4's n ≤ 7 and 3-value results.
...
Exact complete EFX₀, additive or wider valuations, among results that restrict only which agents value which goods (n, m and values otherwise free):

| | q = 1 | q = 2 | q = ∞ |
|---|---|---|---|
| p = 2 | yes (CFKS) | yes | yes (2606.18665, EC 2026; restricted additive: 2407.05139 Thm 8.1) |
| p ≥ 3 | only with girth ≥ 4 (2608.03171); otherwise EF2X, partial EFX, √2/2-EFX (2407.05139, 2606.26948) | exact only on girth-≥ 4 multi-hypergraphs with a multiplicity condition at one vertex (2608.03171 Thm 2); multiplicity-2 hypergraphs of girth ≥ 3: EF3X, 2/3-EFX (2606.26948) | exact on some instances: girth-≥ 4 multi-hypergraphs (girth of the underlying simple hypergraph) whose multiplicity condition holds at one vertex (2608.03171 Thm 2, which bounds multiplicity at that vertex only); otherwise open (general additive EFX) |

**Among results that restrict only which agents value which goods (any n and m, and general additive or wider values), namely those we read (CFKS; 2407.05139; 2506.09288; 2502.09777; 2406.12413 §§1–2, 4.2; 2606.26948; 2608.03171; 2606.18665; and, from abstracts, 2410.17002, 2412.06513 and 2512.21644, which all have p ≤ 2), every exact EFX₀ result with goods of three or more valuers assumes hypergraph girth ≥ 4 (2608.03171).** TARGET needs no girth or multiplicity condition. Exact existence results that bound n, m, the number of valuation types or the values themselves do allow p ≥ 3 (`proofs/novelty.md` §1–§2). They are:
- EFX₀ for n = 3 (Chaudhury–Garg–Mehlhorn), for n = 4 with m ≤ 9 (Alkassar–Fouz–Mehlhorn) and for m ≤ n + 3 (Mahara);
- EFX for two valuations (Mahara), for three valuation types, and for binary or bivalued values; for these the fairness notion was not checked.

Our own T5 allows p ≥ 3 too.

## 3. Relation to this project

### 3.1 Implications, overlaps, contradictions
- **TARGET and D (k = 3): not implied.**
  - Among the structural results (§2.7), exact results need p ≤ 2 or girth ≥ 4. The results that bound n, m, types or values cover only the subclasses listed in `proofs/novelty.md` §1. TARGET cores have goods with three or more valuers and short Berge cycles. Take the showcase core of `proofs/novelty.md` §1: good 0 has three valuers (p = 3), and agents 0 and 1 share goods 0 and 1 (q = 2, so it is not even (∞, 1)).
...
- **Overlap with T3/T5.**
  ...
  - T5 extends the multigraph theorem to goods with three or more valuers under a popular-matching condition. Among the structural results we read, the only exact ones with p ≥ 3 need girth ≥ 4 (2608.03171). T5's class allows short cycles (for example two agents sharing two goods), so it is not covered by them.
- **What TARGET adds to the (p, q) picture.**
  ...
  - More generally, it is exact for p ≥ 3 and any q, with no girth or multiplicity condition. Among results that restrict only which agents value which goods (any n and m, and general additive or wider values), namely those we read (CFKS; 2407.05139; 2506.09288; 2502.09777; 2406.12413 §§1–2, 4.2; 2606.26948; 2608.03171; 2606.18665; and, from abstracts, 2410.17002, 2412.06513 and 2512.21644, which all have p ≤ 2), every exact EFX₀ result with goods of three or more valuers assumes hypergraph girth ≥ 4 (2608.03171). Exact existence results that bound n, m, the number of valuation types or the values do allow p ≥ 3 (`proofs/novelty.md` §1–§2; EFX₀ for n = 3, n = 4 with m ≤ 9, and m ≤ n + 3), and so does T5.
...
- **K4.MC7.** It uses the multigraph theorem for 6 graphical all-P4 cores (n = 6, m = 15, `k4/MINCEX.md` §8). We checked their structure, from the lists in `k4/MINCEX.md` §8:
  ...
  So the external dependency of K4.MC7 can be narrowed to 2 of the 6 cores; for the other four, an older and independent (partly refereed) theorem suffices. This is an observation for the k = 4 workstream, not a ledger change. The structural facts are computed by `results/pq_structure.py` (log `results/pq_structure.log`).
- **Instances with ≤ 4 goods per agent.** Among the structural results we read (§2.7), they are covered exactly in two cases: when p ≤ 2 (any k; the multigraph theorem, as K4.MC7 uses) and when the hypergraph has girth ≥ 4 (any k; 2608.03171). For q = 1 in general only EF2X/partial/approximate results are known. None of these is conditional on k. The results that bound n, m, types or values (`proofs/novelty.md` §1–§2) cover further subclasses; for example, k = 4 instances with n ≤ 3, or with m ≤ n + 3.
...
4. **Minor.**
   ...
   - "Most envious agent" (2.1, §5.4) is attributed in the literature to Chaudhury et al. [unverified: not in anything read here].
   - Parking at safe sets (CFKS Lemma 13) is our junk placement.

### 3.3 Is (p, q) a sensible next direction?
Combining the bounds gives a two-parameter table (k = max goods per agent, p = max agents per good). With n, m and the additive values unrestricted, exact EFX₀ is known for k ≤ 3 with any p (TARGET, this project) and for p ≤ 2 with any k (the multigraph theorem). **The first open cell is (k, p) = (4, 3).** TARGET₄ is (4, ∞).
- **(4, 3) avoids the known obstruction.** Every H_t of `k4/c4.md` §7 has p = 4 (the goods g_j have four valuers) and q = 1. `results/pq_structure.py` computes this from the definition for t ≤ 5 (log `results/pq_structure.log`): H_t is a linear hypergraph with Berge girth 3. So:
