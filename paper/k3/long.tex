...
\begin{itemize}
\item \emph{Lean-checked theorem}: its proof is written in this paper (for the operation bound of Theorem~\ref{thm:algo}, as a step-by-step outline in Appendix~\ref{app:cost}), and its statement is checked in Lean, over the trusted base of Section~\ref{sec:trusted}.
...
\end{itemize}
Every numbered lemma, theorem, proposition and corollary of Sections~\ref{sec:prelim}--\ref{sec:cert} is a Lean-checked theorem unless its heading gives another label. Lean checks these statements for natural-number values, except Theorems~\ref{thm:target} and~\ref{thm:D} and Lemma~\ref{lem:L12}, which it checks for any linearly ordered cancellative commutative monoid, as just explained; the written proofs apply verbatim to real values. Appendix~\ref{app:lean} names the Lean declarations.

\begin{table}[tbp]
\caption{The claims of this paper and their evidence (Section~\ref{sec:answer}). $^*$Lean-checked for values in any linearly ordered cancellative commutative monoid; that $\mathbb{R}_{\ge0}$ is such a monoid is a standard fact, not a Lean theorem (core Lean has no real numbers).}\label{tab:status}
\centering\small
\setlength{\tabcolsep}{4pt}\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.47\textwidth}ll@{}}
\toprule
...
Lem.~\ref{lem:threat}, \ref{lem:L5}, Cor.~\ref{cor:k2}: threats, safety, two goods & $\mathbb{R}_{\ge0}$ & theorem (proof in this paper) \\
...
Cor.~\ref{cor:rat}: rational values & $\mathbb{Q}_{\ge0}$ & written, not formalized \\
Cor.~\ref{cor:real}: real values, comparison oracle & $\mathbb{R}_{\ge0}$ & written, not formalized \\
Sect.~\ref{sec:finer}: $O(n^4+n^2m)$ operations & $\mathbb{N}$ & written, not formalized \\
...
\begin{corollary}[two relevant goods; theorem, proof in this paper]\label{cor:k2}
...
\noindent The existence of an \efxz{} allocation under the hypothesis of Corollary~\ref{cor:k2} is a Lean-checked theorem (\lean{EFX.exists_efx0_of_count}). When rule R1 applies to no agent, every agent has exactly the structure that the rest of the algorithm needs.
...
Output & A complete allocation. & \efxz{} when every agent values at most three goods; at most one bundle of Stage L has more than two goods; at most $400(n+m+1)^4$ unit-cost operations (Theorem~\ref{thm:algo}) \\
\bottomrule
\end{tabular}
\end{table}
...
\noindent Let $B^*$ denote $r$'s block. It is the last block, since the leader of any later block would pick its top and so would not be upgraded; this is not needed below and not formalized. In Lean: \lean{EFX.LB.lastOut_exists}, \lean{EFX.LB.lastOut_terminal}.
...
Let $(Y,U)$ be a valid pre-allocation of an instance with $n$ agents and $m$ goods in which every agent values exactly three goods. Then $\omega=m-2n+|\NA|$. If $\omega\ge1$, every completion with owner $o$ gives $o$ at least $\omega+2$ goods, and exactly $\omega+2$ when all slots of the terminals other than $o$ are full.
\end{proposition}
...
\item[(ii)] \emph{Written, not formalized}: the same program performs $O(n^4+n^2m)$ operations (Section~\ref{sec:finer}).
...
\end{enumerate}
Section~\ref{sec:owner-time} explains why the owner test is cheap; Section~\ref{sec:leanbound} describes layer (i) and its cost model; Sections~\ref{sec:bits} and~\ref{sec:ratreal} give written, not formalized, corollaries for the bit model and for rational and real values; and Section~\ref{sec:finer} gives layers (ii) and (iii).
...
\paragraph{Layer (ii): a finer bound (written, not formalized).} Bounding each step by its own parameters instead of $N$ (Appendix~\ref{app:cost}, last column) gives $O(n^4+n^2m)$ operations for the formalized program. This analysis is written, not machine-checked; Lean states only the coarser $400(n+m+1)^4$.
...
\item Proposition~\ref{prop:size}, the size of the large bundle; Proposition~\ref{prop:neg}, the limits of the shape; Lemma~\ref{lem:threat}; and Corollary~\ref{cor:k2} as a statement about serial dictatorship (its existence form is formalized), and the remark that $r$ lies in the last block.
...
The Lean statements of Section~\ref{sec:correct}'s results use the formalization's own definitions of the state of Stage L. Table~\ref{tab:spec} maps each object of Section~\ref{sec:algo} to the Lean definition that formalizes it, so that a reader can compare them. Names are in the namespace \texttt{EFX.LB} unless they start with \texttt{EFX.K3}, and files are in \file{lean/EFX/}. The Lean state is a ranking profile \texttt{P} (the goods $a_i$, $b_i$, $c_i$ of each agent), the lists of agents and goods, the picks \texttt{Y}, a function from agents to optional goods, and the list \texttt{up} of upgraded agents; most objects below are functions of this state. Where no single Lean definition corresponds to an object, the table says so.
...
blocks, leaders & \lean{blkAux} (block number), \lean{leadB} (leader test); what the proofs use of them: \lean{Run} & \texttt{Blocks} \\
...
\paragraph{Version.} The files named here are those of commit \texttt{a39b151} of the repository~\cite{Repo}, the commit cited in the bibliography; the certificates, scripts and Lean sources are unchanged between that commit and the version of this paper. The repository has no release tag.
