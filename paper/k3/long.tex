...
\begin{abstract}
An allocation of indivisible goods is \efxz{} if no agent envies another agent's bundle after removing any single good from it, even a good that the envious agent values at zero. We prove that every instance with nonnegative additive valuations in which each agent positively values at most three goods has a complete \efxz{} allocation, and we give a polynomial-time algorithm, K3ALG, that computes one. K3ALG first lets agents leave with a good they value at least as much as all their other remaining goods together. The remaining agents pick goods in a serial dictatorship that serves agents who have lost a good first; the leftover goods fill free slots, and the surplus goes to a single owner, after at most one rotation of picks along a chain of agents. For natural-number values, the correctness of K3ALG and a polynomial bound on its number of operations in an explicit unit-cost model are machine-checked in Lean~4 on the program itself; a bound on bit operations is written, not formalized. The existence results are machine-checked for values in an abstract ordered additive structure, any linearly ordered cancellative commutative monoid; instantiating them at the nonnegative reals $\mathbb{R}_{\ge0}$ uses the standard fact that $\mathbb{R}_{\ge0}$ is such a monoid (core Lean has no real numbers). If every agent values exactly three goods, none more than the other two together, there is a complete \efxz{} allocation in which all but at most one agent receive at most two goods. This long version is meant to be read on its own: it explains the approach informally, traces the algorithm on two examples, and gives every proof of the main theorems in full.
...
We allocate a set $M$ of $m$ indivisible goods to a set $N$ of $n$ agents. Agent $i$ has an \emph{additive valuation}: a value $v_i(g)\ge0$ for each good $g$, and $v_i(S)=\sum_{g\in S}v_i(g)$ for every set $S$ of goods. An \emph{allocation} $X$ is a function that assigns every good to exactly one agent; agent $i$'s \emph{bundle} $X_i$ is the set of goods assigned to $i$. So the bundles are pairwise disjoint and cover $M$: every allocation in this paper is \emph{complete}, and no good may be thrown away.
...
\end{align*}
Both conditions are vacuous when $X_j$ is empty. \efxz{} implies EFX, and the two coincide when all values are positive. Whether every instance with additive valuations has a complete EFX allocation is a central open question of fair division~\cite{AmanatidisBFV22}. Most known existence results (Section~\ref{sec:related}) are proved in the stronger form \efxz{}. The one rule that separates the two notions, and that this paper is about, is the following.
\begin{center}
\fbox{\parbox{0.92\textwidth}{EFX and \efxz{} differ only in which goods may be removed from the other bundle. \emph{Under \efxz{}, if another agent's bundle contains a good that agent $i$ values at $0$, then $i$ must not envy that bundle at all.}}}
\end{center}
...
Every instance with $n\ge1$ agents and nonnegative real additive valuations in which every agent values exactly three goods and is balanced has a complete \efxz{} allocation in which all but at most one agent receive at most two goods.
\end{theorem}

\noindent The third result is an algorithm. We measure its running time in an explicit \emph{unit-cost model}, where each list step, comparison of agents or goods, read of an input value or table entry, and comparison or addition of natural numbers of any size costs one operation (Section~\ref{sec:time} states the model precisely; Corollary~\ref{cor:bits}, written, not formalized, counts bit operations instead).
...
\begin{corollary}[rational values]\label{cor:rat}
Let every agent positively value at most three goods, with nonnegative rational values. Scale each agent's values to natural numbers, multiplying them by a common denominator; this preserves every comparison between sums of that agent's values. Then K3ALG on the scaled values returns a complete allocation that is \efxz{} for the rational values.
\end{corollary}

\noindent The proofs of Theorems~\ref{thm:target}--\ref{thm:algo} are written in full in this paper. They are also checked in Lean~4 (Section~\ref{sec:lean}): Theorem~\ref{thm:algo} for natural-number values, and Theorems~\ref{thm:target} and~\ref{thm:D} for values in any linearly ordered cancellative commutative monoid; so are Corollary~\ref{cor:rat}, for nonnegative rationals, and Corollary~\ref{cor:real}, K3ALG on real values in a comparison model, for any such monoid. Instantiating these at $\mathbb{R}_{\ge0}$ uses the standard fact that $\mathbb{R}_{\ge0}$ is such a monoid, which is not a Lean theorem, since core Lean has no real numbers.

\paragraph{Kinds of evidence.} Every claim of this paper carries exactly one of five labels, and Table~\ref{tab:status} lists the claims with their labels.
\begin{itemize}
\item \emph{Lean-checked theorem}: its proof is written in this paper (for the operation bounds of Theorems~\ref{thm:algo} and~\ref{thm:fine}, as a step-by-step outline in Appendix~\ref{app:cost}), and its statement is checked in Lean, over the trusted base of Section~\ref{sec:trusted}.
\item \emph{Theorem (proof in this paper)}: the proof is written in this paper; it is not formalized.
\item \emph{Written, not formalized}: a written argument that relies on material outside this paper's definitions, namely the text of a program (for counts of its operations and what they compute) or a proof recorded in the repository; it is not formalized.
\item \emph{Verified by exhaustive computation}: every case is checked by programs, and the stored certificates are re-checked by an independently written checker.
\item \emph{Measurement}: observed in runs of implementations; evidence, not a guarantee.
\end{itemize}
Every numbered lemma, theorem, proposition and corollary of this paper is a Lean-checked theorem unless its heading gives another label. Lean checks these statements for natural-number values, except Theorems~\ref{thm:target} and~\ref{thm:D}, Lemma~\ref{lem:L12} and Corollary~\ref{cor:real}, which it checks for any linearly ordered cancellative commutative monoid, as just explained; Corollary~\ref{cor:rat}, which it checks for nonnegative rationals; and Proposition~\ref{prop:size}, which concerns the rankings alone. The written proofs apply verbatim to real values. Appendix~\ref{app:lean} names the Lean declarations.

\begin{table}[tbp]
\caption{The claims of this paper and their evidence (Section~\ref{sec:answer}). $^*$Lean-checked for values in any linearly ordered cancellative commutative monoid; that $\mathbb{R}_{\ge0}$ is such a monoid is a standard fact, not a Lean theorem (core Lean has no real numbers). \emph{Rankings}: the claim involves only the agents' rankings, whatever the values.}\label{tab:status}
\centering\small
\setlength{\tabcolsep}{4pt}\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.44\textwidth}ll@{}}
\toprule
Claim & Values & Evidence \\
\midrule
Thm.~\ref{thm:target}: existence & $\mathbb{R}_{\ge0}$ & Lean-checked theorem$^*$ \\
Thm.~\ref{thm:D}: one large bundle & $\mathbb{R}_{\ge0}$ & Lean-checked theorem$^*$ \\
Thm.~\ref{thm:algo}: K3ALG is correct & $\mathbb{N}$ & Lean-checked theorem \\
Thm.~\ref{thm:algo}: $400(n+m+1)^4$ operations, unit cost & $\mathbb{N}$ & Lean-checked theorem \\
Thm.~\ref{thm:fine}: $270(n^4+n^2m)$ operations, unit cost & $\mathbb{N}$ & Lean-checked theorem \\
Sects.~\ref{sec:prelim}, \ref{sec:correct}: the other lemmas and theorems & $\mathbb{N}$ & Lean-checked theorem \\
Cor.~\ref{cor:k2}: serial dictatorship, two goods & $\mathbb{N}$ & Lean-checked theorem \\
Prop.~\ref{prop:size}: size of the large bundle & rankings & Lean-checked theorem \\
Sect.~\ref{sec:rexp}: $r$ lies in the last block & rankings & Lean-checked theorem \\
Cor.~\ref{cor:rat}: rational values & $\mathbb{Q}_{\ge0}$ & Lean-checked theorem \\
Cor.~\ref{cor:real}: real values, comparison oracle & $\mathbb{R}_{\ge0}$ & Lean-checked theorem$^*$ \\
Lem.~\ref{lem:threat}, \ref{lem:L5}: threats, safety & $\mathbb{R}_{\ge0}$ & theorem (proof in this paper) \\
Prop.~\ref{prop:neg}: limits of the shape & $\mathbb{R}_{\ge0}$ & theorem (proof in this paper) \\
Cor.~\ref{cor:bits}: bit complexity & $\mathbb{N}$ & written, not formalized \\
Sect.~\ref{sec:finer}: Python, $O((n+m)\log n)$ after the input & $\mathbb{N}$ & written, not formalized \\
Sect.~\ref{sec:cert}: existence for $n\le7$ & $\mathbb{R}_{\ge0}$ & verified by exhaustive computation \\
App.~\ref{app:cert}: Thm.~\ref{thm:D}, cores with $n\le6$, connected $n=7$ & $\mathbb{R}_{\ge0}$ & verified by exhaustive computation \\
Sect.~\ref{sec:finer}: speed of the Python implementation & $\mathbb{N}$ & measurement \\
App.~\ref{app:cert}: tests of \LBp{} and K3ALG & $\mathbb{N}$ & measurement \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Why this is not a corollary of known results.} A $3$-relevant instance without a complete \efxz{} allocation would refute the additive EFX conjecture itself. Indeed, adding a small $\varepsilon>0$ to every value gives an instance with positive values, in which EFX and \efxz{} coincide. If each of these perturbed instances had an EFX allocation, one allocation would serve arbitrarily small $\varepsilon$, since there are finitely many allocations, and it would be \efxz{} for the original values, since the inequalities are closed (compare the perturbation lemma of~\cite{ChaudhuryGM20}). But the perturbation makes every good relevant to every agent, so it destroys the structure we exploit, and no published result that we know of implies Theorem~\ref{thm:target}~\cite{Repo}. Theorem~\ref{thm:target} gives one, with a polynomial-time algorithm whose correctness and operation bound, in an explicit unit-cost model, are machine-checked.

\subsection{Related work}\label{sec:related}

For additive valuations, complete EFX allocations are known to exist only in special cases, two of them in recent preprints~\cite{AlkassarFM26,AfshinmehrAMMS26}: for three agents~\cite{ChaudhuryGM20}; when $m\le n+3$~\cite{Mahara21}; for four agents and at most nine goods~\cite{AlkassarFM26}; when every good is valued by at most two agents (graphs and multigraphs of agents, with goods as edges)~\cite{AfshinmehrAMMS26,ChristodoulouFKS23,SgouritsaS25,KavianiSS24}; and for hypergraphs of girth at least four~\cite{LianeasSS26}. All of these use the all-goods form \efxz{}; in the graph and hypergraph results every good is worth zero to the agents it is not incident to, and for results that bound $n$ and $m$ positive values suffice by the perturbation argument above. For monotone valuations that are not additive, EFX allocations need not exist~\cite{AkramiMMSW26,MackenzieS26}. Table~\ref{tab:rw} compares these results with ours.

We restrict instead how many goods each agent values. For $k\le2$, serial dictatorship suffices and has been formalized in Lean~\cite{MrdEfx}. For $k\le4$, Viswanathan and Mehta~\cite{ViswanathanM24} announce the existence of ordinary EFX allocations, with a polynomial-time algorithm and a proof sketch in an extended abstract. Their first phase, which gives a first choice, by a maximum matching, to as many as possible of the agents that value their top good more than their second and third together, resembles our rule R1 (Section~\ref{sec:peel}). Their algorithm ends by giving the goods that nobody values to an arbitrary agent, which is harmless for EFX but can break \efxz{}, as Example~\ref{ex:efx-vs-efx0} illustrates. Goods that nobody values are in fact easy to place (Lemma~\ref{lem:junk}); the difficulty under \efxz{} comes from goods that one agent values and another does not, which their EFX argument does not need to address. Since \efxz{} implies EFX, Theorem~\ref{thm:target} proves the case $k=3$ of their result in full.

\begin{table}[tbp]
\caption{Existence results for complete allocations under restrictions, from the reading notes in \texttt{proofs/citations.md} of~\cite{Repo}. \emph{Version cited}: published version if Crossref or the paper confirms one, else arXiv. $^a$Goods nobody values go to an arbitrary agent. $^b$Includes additive. $^c$Extended abstract with a proof sketch. $^d$An AAMAS 2025 extended abstract has the other two cases.}\label{tab:rw}
...
\cite{AfshinmehrAMMS26} & multigraphs: at most two valuers per good & \efxz{} & yes & cancelable$^b$ & polynomial time & arXiv \\
...
this paper & at most three relevant goods per agent; any number of valuers per good & \efxz{} & yes & additive & $400(n+m+1)^4$ unit-cost steps; Lean-checked & this paper \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Where the difficulty lies.} Under ordinary EFX, a good that agent $i$ values at zero never constrains $i$, so goods valued by some agents and not others are harmless; this is why~\cite{ViswanathanM24} can give the goods nobody values to an arbitrary agent, and on multigraphs ordinary EFX is easy, so the difficulty there is \efxz{}~\cite{SgouritsaS25}. Under \efxz{} with complete allocations, every such good must go into some bundle, and it turns the constraint of each agent that does not value it into envy-freeness towards that bundle (the rule of Section~\ref{sec:intro-efx}). \emph{The difficulty of Theorem~\ref{thm:target} is exactly this combination: goods that some agents value and others do not, together with completeness.} The structural results of Table~\ref{tab:rw} cope with it when each good has at most two valuers or the valuers form a hypergraph of girth at least four; the others bound $n$ or $m$. Theorem~\ref{thm:target} bounds only the number of goods each agent values: a good may have any number of valuers, and any number of agents to whom it is worthless.

\subsection{How to read this paper}\label{sec:roadmap}

Section~\ref{sec:overview} explains the approach in plain words on one instance, with no formal definitions. Section~\ref{sec:prelim} gives the model and four basic lemmas: the ordinal description of when an agent is safe (Lemma~\ref{lem:L5}), on which the whole design rests; peeling; goods valued by nobody; and the reduction of real values to natural numbers. Section~\ref{sec:algo} defines the algorithm K3ALG one notion at a time, lists it as pseudocode, and traces it on two examples, one with an owner and one with a rotation. Section~\ref{sec:correct} proves it correct, with every case written out. Section~\ref{sec:time} bounds its running time, in three separate layers. Section~\ref{sec:lean} describes the Lean formalization: what it covers, what must be trusted, how to read its statements, and what it leaves out. Section~\ref{sec:cert} explains what the exhaustive computations for small instances establish and shows, by counterexamples, that the shape of Theorem~\ref{thm:D} cannot be improved in the obvious ways. Section~\ref{sec:concl} concludes. The appendices hold material for checking the artifacts rather than for reading: the cost of each step, the verification of the examples, the repository's audit of the formalization, a table that maps every object of Section~\ref{sec:algo} to its Lean definition, and the run totals, certificates and reproducibility details of the computations.

The paper is self-contained. Its claims are also recorded, each with its evidence, in the ledger of the public repository~\cite{Repo}. Every example in this paper was checked by running the repository's implementations of K3ALG with a brute-force check of the \efxz{} definition, and by the Lean program itself (Appendix~\ref{app:examples}).
...
\paragraph{Step 3: why the leftovers are the whole difficulty.} If no good were left over, every agent would hold at most one good, and the allocation would be \efxz{} for the trivial reason that nobody can strongly envy a bundle of at most one good. The leftovers, which we call \emph{junk}, must go somewhere, and in general they create bundles of two or more goods, which can be strongly envied. Here the two notions part ways. Under ordinary EFX, adding to a bundle a good that agent $i$ does not value never creates a new constraint for $i$, since only goods that $i$ values may be removed. Under \efxz{}, it turns $i$'s constraint towards that bundle into envy-freeness, as in Example~\ref{ex:efx-vs-efx0}. Lemma~\ref{lem:L5} says exactly when a strictly balanced agent with $v(a)>v(b)>v(c)$ is safe:
\begin{itemize}
...
\paragraph{Step 5: the owner.} When the junk exceeds the slots, one agent, the \emph{owner}, takes all the junk that does not fit, so its bundle has three or more goods. (Usually every other slot is then filled; when the list of protecting goods that K3ALG builds repeats a good, one slot can stay empty and the owner's bundle is larger, Remark~\ref{rem:repeat}.) By Lemma~\ref{lem:L5}, and because goods needed alone never go to the owner, such a bundle can hurt only an agent $x$ that holds its top $a_x$ and would find both $b_x$ and $c_x$ in it. Such an agent is \emph{exposed}, and it is protected if one of $b_x$, $c_x$ goes into a slot instead. The natural candidate owner is $r$, the last agent served that is not upgraded; here $r=1$. Agent~$0$ is exposed for $r$: it holds its top $g_2$, its $b=g_4$ is junk, and its $c=g_6$ is $r$'s own pick. So $g_4$ goes into agent~$3$'s slot and the owner takes $g_0$ and $g_7$:
\[
...
\begin{lemma}[threats; theorem, proof in this paper]\label{lem:threat}
...
\begin{lemma}[cases of safety; theorem, proof in this paper]\label{lem:L5}
...
If $|R_i|\le2$ for every agent, then serial dictatorship, in any order of the agents, each taking a favourite remaining good (any good of largest value to it among those left, or nothing once no good is left), with the last agent taking all remaining goods, returns an \efxz{} allocation.
\end{corollary}

\begin{proof}
By induction on the number of agents. One agent faces no constraint. Otherwise the first agent $i$ has at most two relevant goods among the remaining ones, and takes a favourite remaining good $p$; its other relevant good, if any, is worth at most $v_i(p)$, so $v_i(M\setminus\{p\})\le v_i(p)$. The rest of the procedure is serial dictatorship on the smaller instance, which is \efxz{} by induction, and Lemma~\ref{lem:peel} with $P=\{p\}$ finishes the proof; if no good is left, $i$ takes $P=\emptyset$, and (i) reads $0\ge0$. \qed
\end{proof}

\noindent In Lean, for natural-number values: \lean{EFX.Inst.sdRun_efx0} (every run: any order of all agents, any choice of favourite at each step) and \lean{EFX.Inst.serialDict_efx0} (a computable run, taking the first favourite, in any order); the existence form is \lean{EFX.exists_efx0_of_count}. The form for values in an ordered type is not formalized. When rule R1 applies to no agent, every agent has exactly the structure that the rest of the algorithm needs.
...
\noindent The written proofs of Theorems~\ref{thm:target} and~\ref{thm:D} below work verbatim for real values. Lemma~\ref{lem:L12} is what carries the Lean proofs, which are about natural numbers, over to any linearly ordered cancellative commutative monoid, and so, by the standard fact that $\mathbb{R}_{\ge0}$ is one, to real values (Section~\ref{sec:lean}); in Lean it is \lean{EFX.l12}, with \lean{EFX.efx0_iff_of_agree} for the transfer of \efxz{}.
...
\paragraph{Input.} As formalized, K3ALG receives $n\ge1$, $m$ and the valuation as a function $v$ that gives each agent $i$ and good $g$ a value $v_i(g)\in\mathbb{N}$ (the field \texttt{v} of the model's instance, Section~\ref{sec:trusted}). The program reads it as a dense $n\times m$ table, at one unit per read of a value $v_i(g)$ (Section~\ref{sec:leanbound}). Correctness needs every agent to value at most three goods; the operation bound holds for every input. Table~\ref{tab:glance} summarizes the algorithm and the invariant behind each of its steps.

\begin{table}[tbp]
\caption{K3ALG at a glance. Each invariant is proved at the place named in parentheses.}\label{tab:glance}
\centering\small
\setlength{\tabcolsep}{4pt}\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.14\textwidth}>{\raggedright\arraybackslash}p{0.44\textwidth}>{\raggedright\arraybackslash}p{0.36\textwidth}@{}}
\toprule
Step & What it does & Invariant \\
\midrule
Input & $n\ge1$ agents, $m$ goods, a table of values $v_i(g)\in\mathbb{N}$. Correctness assumes that every agent values at most three goods; the operation bound does not. & -- \\
Stage R & Peel by rule R1: an agent whose favourite remaining good is worth at least all its other remaining goods takes it and leaves; an agent that values no remaining good leaves with nothing. & a peeled agent is safe, and nobody strongly envies its bundle (Lemma~\ref{lem:peel}); afterwards every agent values exactly three goods and is strictly balanced (Lemma~\ref{lem:R1fail}) \\
Phase 1 & Serial dictatorship with R1 priority: an agent that has lost a good goes first; otherwise a new leader takes its top. Gives the picks, blocks and leaders. & losses stay inside a block: every good an agent ranks above its pick was picked earlier in its own block ((B2), Lemma~\ref{lem:inv}) \\
Upgrades & An agent that picked its $b$, whose $c$ is junk and whose $b$ nobody needs alone, also takes its $c$. & junk and upgraded goods are not needed alone: the state is valid (Lemma~\ref{lem:upgrades}) \\
Completion & Picks to their pickers, $c_u$ to each upgraded $u$, junk into the slots of the terminals, the rest of the junk to the owner, if any. & all non-owner bundles have at most two goods (Theorem~\ref{thm:sound}) \\
Owner test & If the junk exceeds the slots: $r$, the last agent not upgraded, is a valid owner iff $\HitSet(E_r)$ fits the other agents' slots; then complete with owner $r$, putting $\HitSet(E_r)$ into slots. & owner constraint: no agent that holds its top $a_x$ and is not upgraded finds both $b_x$ and $c_x$ in the owner's bundle (Lemma~\ref{lem:owner}, Proposition~\ref{prop:O}) \\
Rotation & Otherwise, rotate the picks along the need chain from $k^*$ to $r$; $k^*$ takes $b_{k^*}$ and $c_{k^*}$. Complete without owner if the junk now fits, else with owner $k^*$. & $\NA$ shrinks ($\NA'\subseteq\NA$) and $k^*$ becomes upgraded; the state stays valid, and $k^*$ is a valid owner if the junk still exceeds the slots (Theorem~\ref{thm:B}) \\
Output & A complete allocation. & \efxz{} when every agent values at most three goods; at most one bundle of Stage L has more than two goods; at most $400(n+m+1)^4$ and at most $270(n^4+n^2m)$ unit-cost operations (Theorems~\ref{thm:algo} and~\ref{thm:fine}) \\
\bottomrule
\end{tabular}
\end{table}
...
We trace K3ALG on two instances. For each we give the state after every stage, summarized in Tables~\ref{tab:ex1stages} and~\ref{tab:ex2stages}, and check the output by hand. Both were also checked by machine (Appendix~\ref{app:examples}).

\begin{table}[tbp]
\caption{Example~\ref{ex:owner}, stage by stage.}\label{tab:ex1stages}
\centering\small
\setlength{\tabcolsep}{4pt}\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.17\textwidth}>{\raggedright\arraybackslash}p{0.79\textwidth}@{}}
\toprule
after & state \\
\midrule
Stage R & agent~$4$ peeled with $g_5$; $A=\{0,1,2,3\}$ \\
Phase 1 & order $0,2,3,1$; picks $Y_0=g_2$, $Y_2=g_1$, $Y_3=g_8$, $Y_1=g_6$; blocks $\{0,2,3\}$ (leader $0$) and $\{1\}$ (leader $1$); leftovers $J_0=\{g_0,g_3,g_4,g_7\}$ \\
upgrades & $U=\{2\}$ (agent~$2$ receives $g_3$); $J=\{g_0,g_4,g_7\}$; $\NA=\{g_2\}$; frozen: $0$; terminals: $1$, $3$, one slot each; $S=2$, $\omega=1$ \\
owner test & $r=1$; $E_1=\{0\}$, $\pi_0=\{g_4\}$; $\HitSet(E_1)=(g_4)$ fits $S-\capa(1)=1$: owner $r=1$ \\
completion & $L=(g_4,g_0,g_7)$: $g_4$ to agent~$3$, and $g_0$, $g_7$ to the owner \\
\bottomrule
\end{tabular}
\end{table}

\begin{example}[owner $r$]\label{ex:owner}
This is the instance of Section~\ref{sec:overview} (agents $0,\dots,4$, goods $g_0,\dots,g_8$, values listed there), now traced with the definitions.
...
\begin{table}[tbp]
\caption{Example~\ref{ex:rot}, stage by stage.}\label{tab:ex2stages}
\centering\small
\setlength{\tabcolsep}{4pt}\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.17\textwidth}>{\raggedright\arraybackslash}p{0.79\textwidth}@{}}
\toprule
after & state \\
\midrule
Stage R & nobody peeled \\
Phase 1 & order $0,1,2,3$; picks $Y_0=g_2$, $Y_1=g_4$, $Y_2=g_7$, $Y_3=g_0$; blocks $\{0\}$ (leader $0$) and $\{1,2,3\}$ (leader $1$); leftovers $J_0=\{g_1,g_3,g_5,g_6\}$ \\
upgrades & none: $U=\emptyset$, $J=J_0$; $\NA=\{g_4,g_7\}$; frozen: $1$, $2$; terminals: $0$, $3$, one slot each; $S=2$, $\omega=2$ \\
owner test & $r=3$; $E_3=\{0,1\}$, $\pi_0=\{g_5\}$, $\pi_1=\{g_1,g_6\}$, disjoint; $\HitSet(E_3)=(g_5,g_6)$ does not fit $S-\capa(3)=1$: bad case, $k^*=1$ \\
rotation & chain $1\to2\to3$: $Y'_2=g_4$, $Y'_3=g_7$, agent~$1$ holds $g_6$ and $g_1$; $U'=\{1\}$; $J'=\{g_0,g_3,g_5\}$; $\NA'=\{g_4\}$; frozen: $2$; terminals: $0$, $3$, one slot each; $S'=2$, $\omega'=1$ \\
completion & owner $k^*=1$; $E'_1=\{0\}$, $\HitSet(E'_1)=(g_5)$; $L=(g_5,g_0,g_3)$: $g_5$ to agent~$0$, $g_0$ to agent~$3$, $g_3$ to the owner \\
\bottomrule
\end{tabular}
\end{table}
...
(A0) Some agent is not upgraded; let $r$ be the last processed agent that is not upgraded. (A1) $r$ is a terminal. (A2) Every agent processed after $r$ is upgraded.
\end{lemma}
...
(A2) This is the choice of $r$. \qed
\end{proof}

\noindent Let $B^*$ denote $r$'s block. It is the last block, since the leader of any later block would pick its top and so would not be upgraded, yet would come after $r$; this is not needed below. In Lean: \lean{EFX.LB.lastOut_exists}, \lean{EFX.LB.lastOut_terminal}; and for the last block, \lean{EFX.LB.lastOut_lastBlock} (no agent's block exceeds $r$'s, and $r$'s block is that of the last processed agent), from the general form \lean{EFX.LB.blk_le_lastOut}, which needs only that no upgraded agent picks its top.
...
\begin{proposition}[size of the large bundle]\label{prop:size}
Let $(Y,U)$ be a valid pre-allocation of an instance with $n$ agents and $m$ goods in which every agent values exactly three goods. Then $\omega=m-2n+|\NA|$. Every completion with owner $o$ gives $o$ at least $\omega+2$ goods, and exactly $\omega+2$ when all slots of the terminals other than $o$ are full.
\end{proposition}
...
\]
The other terminals receive at most $S-\capa(o)$ junk goods, exactly that many when their slots are full, so the owner receives its base and at least $|J|-(S-\capa(o))=\omega+\capa(o)$ junk goods, with equality when the other slots are full; and $|\base(o)|+\capa(o)=2$ for every terminal and every upgraded agent. \qed
\end{proof}

\noindent An owner is needed only when $\omega\ge1$, but the count and both bounds hold for every $\omega$. The rotation does not increase $\NA$ (Theorem~\ref{thm:B}), so it does not enlarge the large bundle. In Example~\ref{ex:owner}, $m-2n+|\NA|=8-8+1=1=\omega$ for the Stage-L instance, and the owner holds $3$ goods. In Lean: \lean{EFX.LB.largeBundle_size}, from \lean{EFX.LB.omega_eq}, \lean{EFX.LB.numFrozen_eq_numNA}, \lean{EFX.LB.Completion.owner_length_ge} and \lean{EFX.LB.Completion.owner_length_eq}; for the rotation, \lean{EFX.LB.BadCase.omega_le}.

\begin{remark}[a repeated good]\label{rem:repeat}
K3ALG's completion fills every slot of the terminals other than the owner when $\HitSet$ lists no good twice, and then Proposition~\ref{prop:size} gives the size of its large bundle, $\omega+2$ (in Lean, \lean{EFX.LB.complete_owner_length}, for $\omega\ge1$ and the hypotheses of Lemma~\ref{lem:owner}). A good can be listed twice, and then a slot stays empty and the owner receives more goods; the output is still \efxz{} by Lemma~\ref{lem:owner}. For example, let four agents value $g_0$ at $3$, and agent $i$ also value $g_{2i+1}$ at $4$ and $g_{2i+2}$ at $2$ ($i=0,\dots,3$, nine goods). All four agents are leaders and pick their tops, $\NA=\emptyset$, every agent is a terminal with one slot, $S=4$ and $|J|=5$, so $\omega=1$. The owner is $r=3$, with $E_3=\{0,1,2\}$ and $\HitSet(E_3)=(g_0,g_0)$: the pair of agents $0$ and $1$ shares $g_0$, and agent~$2$'s junk $b$ is $g_0$ again. So $L=(g_0,g_0,g_2,g_4,g_6,g_8)$: agent~$0$ receives $g_0$, agent~$1$'s window holds only the repeated $g_0$, agent~$2$ receives $g_2$, and the owner receives $g_4$, $g_6$ and $g_8$ besides its pick $g_7$: four goods in all, which is $\omega+3$. Lean checks this run of K3ALG, its state and the bundle sizes by \texttt{decide}, and that the output is \efxz{} (\lean{EFX.K3.Examples.repeatedGood_state}, \lean{EFX.K3.Examples.repeatedGood_algo}).
\end{remark}

\section{Running Time}\label{sec:time}

Running-time claims come in three layers, which we keep apart.
\begin{enumerate}
\item[(i)] \emph{Lean-checked}: on every input, the formalized program K3ALG performs, in the unit-cost model of Section~\ref{sec:leanbound}, at most $400(n+m+1)^4$ operations (Theorem~\ref{thm:time}). It takes the valuation as a function $v$, read as a dense $n\times m$ table at one unit per value.
\item[(ii)] \emph{Lean-checked}: the same program performs at most $n^4+20n^3+25n^2m+124n^2+47nm+119n+22m+3\le270(n^4+n^2m)$ operations, in the same cost model (Theorem~\ref{thm:fine}, Section~\ref{sec:finer}).
\item[(iii)] \emph{A separate implementation}: an optimized Python program, not verified, for which a written analysis, not formalized, gives $O((n+m)\log n)$ operations after the input is read, and whose speed is measured (Section~\ref{sec:finer}). Reading a dense $n\times m$ table already costs $nm$ operations, so (iii) excludes the reading of the input; the implementation receives each agent's relevant goods as a list.
\end{enumerate}
Section~\ref{sec:owner-time} explains why the owner test is cheap; Section~\ref{sec:leanbound} describes layer (i) and its cost model; Section~\ref{sec:bits} gives a written, not formalized, corollary for the bit model, and Section~\ref{sec:ratreal} Lean-checked corollaries for rational and real values; and Section~\ref{sec:finer} gives layers (ii) and (iii).
...
\subsection{Layer (i): the Lean-checked operation bound}\label{sec:leanbound}
...
Multiply agent $i$'s values by a common denominator of them. This multiplies every sum of $i$'s values by the same positive number, so every comparison between two such sums is preserved, and with it the relevant goods and the \efxz{} condition, as in Lemma~\ref{lem:L12}. So the scaled instance is $3$-relevant, K3ALG's output on it is \efxz{} for the scaled values (Theorem~\ref{thm:algo}), and hence for the rational values. \qed
\end{proof}

\noindent In Lean: \lean{EFX.K3.algoRat_efx0}, for core Lean's rationals, with \lean{EFX.K3.algoRat}, K3ALG on \lean{EFX.ratScale}, which multiplies agent $i$'s values by the product of their denominators (\lean{EFX.ratScale_val}); \lean{EFX.ratScale_agree} is the preservation of comparisons, and \lean{EFX.ratScale_efx0_iff} and \lean{EFX.ratScale_numRelevant} its consequences. Every decision of K3ALG is also the same on the scaled values as it would be on the rational values, since each compares two sums of one agent's values; this remark, and the cost of computing the scaling, are written, not formalized.

For real values, the model must say what an algorithm can do with a real number. We use a comparison oracle, in the style of the real RAM.

\begin{corollary}[real values, comparison model]\label{cor:real}
Let every agent positively value at most three goods, with nonnegative values in $\mathbb{R}_{\ge0}$, or in any linearly ordered cancellative commutative monoid. Suppose an oracle answers at unit cost whether $x\le y$, where $x$ and $y$ are sums of at most two values of one agent (the same value twice allowed for agents with one or two relevant goods), and adding two values of one agent costs one operation. Then a program computes, with at most $n(m+12)$ oracle calls, natural-number valuations $w$ that agree with $v$ on every comparison of subset sums, as in Lemma~\ref{lem:L12}, and K3ALG on $w$ returns an allocation that is \efxz{} for $v$. In all it performs at most $n(m+12)+10nm+971n+400(n+m+1)^4$ operations, oracle calls included, and at most $n(m+12)+10nm+971n+145n^4+72n^2m+119n+22m+3$.
\end{corollary}

\begin{proof}
The program inspects the values only through the oracle, and it adds values of one agent. For each agent $i$ it finds $R_i$, in index order, with $m$ calls $v_i(g)\le0$. Let $x_1,x_2,x_3$ be the values of $i$'s first three relevant goods; with one relevant good, $x_2=x_3=x_1$, and with two, $x_3=x_1$ (a phantom slot). The program makes six additions and twelve calls: $x_j\le x_k$ for $j\ne k$, and $x_j\le x_k+x_l$ and $x_k+x_l\le x_j$ for $\{j,k,l\}=\{1,2,3\}$. These answers determine the ties and the sign of each $x_j-(x_k+x_l)$, which, as in the proof of Lemma~\ref{lem:L12} (Appendix~\ref{app:L12}), decide every comparison of two subset sums of the relevant goods. The program sets $w_i$ on these goods to the first of the $31$ natural-number triples of the Lean proof of Lemma~\ref{lem:L12} (all permutations of the representatives of Appendix~\ref{app:L12}) that gives the same answers, and $w_i(g)=0$ elsewhere, and stores $w$ as an $n\times m$ table. The entries of $w$ are at most $5$. With one or two relevant goods, $w_i$ may differ from the representatives listed in Appendix~\ref{app:L12} (for values $(1,3)$ it is $(1,3)$) and still agrees with $v_i$. Per agent this is $m+12$ oracle calls and $O(m)$ other operations: at most $n(m+12)+10nm+971n$ in all. Then K3ALG on $w$ takes at most $400(n+m+1)^4$ operations (Theorem~\ref{thm:time}), and at most $145n^4+72n^2m+119n+22m+3$ (Theorem~\ref{thm:fine}), with numbers of $O(\log m)$ bits; its output is \efxz{} for $w$ (Theorem~\ref{thm:algo}), hence for $v$ (Lemma~\ref{lem:L12}). \qed
\end{proof}

\noindent In Lean: \lean{EFX.K3.algoOrd_efx0} (correctness, assuming a correct oracle), \lean{EFX.K3.algoOrdC_cost} and \lean{EFX.K3.algoOrdC_cost_fine} (the two operation bounds), with \lean{EFX.K3.surrogateC_cost} (the surrogate's cost; its coefficient of the cost of a call bounds the calls), \lean{EFX.K3.agree_surrogate} (the surrogate agrees with $v$) and \lean{EFX.K3.algoOrd_eq} (the program is K3ALG on the surrogate). The values may lie in any type of the class \lean{EFX.OrderedValue}, and that $\mathbb{R}_{\ge0}$ is one is the standard fact of Section~\ref{sec:trusted}. The written argument needs only comparisons of two subset sums, at most six per agent after the $m$ comparisons with $0$ (three to sort, the ties, and the top against the other two; Appendix~\ref{app:L12}); the program's comparisons for agents with one or two relevant goods, such as $v_i(g_2)\le2v_i(g_1)$, are not of that kind, which is why the oracle of the corollary compares sums of at most two values with repetition.

Alternatively, K3ALG can run on real inputs directly. Every decision it makes compares two sums of one agent's values (relevance, the favourite good, rule R1, and the three comparisons of the sort), and Lemma~\ref{lem:L12} answers each such comparison for $v$ as for $w$, so the run on $v$ is the run on $w$ and returns the same allocation. This remark is written, not formalized.

\subsection{A finer bound and measurements}\label{sec:finer}

\paragraph{Layer (ii): a finer bound.} Bounding each step by its own parameters instead of $N$, with a bound $n$ on every list of agents ($n+1$ for the upgraded agents after the rotation, $k^*$ included) and $m$ on every list of goods, gives the following (Appendix~\ref{app:cost}, last column). It is proved for the same program and in the same cost model as Theorem~\ref{thm:time}.

\begin{theorem}[finer operation count; \texttt{EFX.K3.algoC\_cost\_fine}]\label{thm:fine}
For every instance with $n\ge1$ agents and $m$ goods, with any natural-number values (no assumption on the number of relevant goods),
\[
  \mathtt{(algoC\ I\ hn).cost}\ \le\ n^4+20n^3+25n^2m+124n^2+47nm+119n+22m+3;
\]
hence $\le145n^4+72n^2m+119n+22m+3$ ({\upshape\lean{EFX.K3.algoC_cost_fine'}}) and $\le270(n^4+n^2m)$ ({\upshape\lean{EFX.K3.algoC_cost_fine''}}).
\end{theorem}

\noindent The dominant terms come from the upgrade loop, at most $n^4+17n^3+n^2m+12n^2+4nm+2n+m$ (\lean{EFX.K3.lbUpC_cost_fine}): at most $n$ rounds, each testing every agent, each test deciding whether $b_k\in\NA$ by scanning the agents with a membership test in $U$ and whether $c_k\in J$. The $n^2m$ terms come from Stage R ($n$ rounds, $n$ agents, an R1 test over the goods), the processing order, Phase~1's picks, the blocks, and the search for two meeting sets $\pi_x$ in both owner tests (pairs of exposed agents, membership in $J$); there is no $nm^2$ or $m^2$ term. Stage L and Stage R are bounded by \lean{EFX.K3.lbPlusC_cost_fine} and \lean{EFX.K3.reduceC_cost_fine}.

\paragraph{Layer (iii): a faster implementation (written, not formalized).} A Python implementation of the same algorithm, the function \texttt{fast} of the file \file{k3/k3algo.py} of the repository, replaces the naive scans by a heap of the agents to which R1 applies (under $3$-relevance, R1 keeps applying to an agent once it applies), a heap of the agents with a good missing (for Phase~1's order), per-good counters of the agents that rank a good above their pick (for the upgrades), and an index from junk goods to exposed agents (for $\HitSet$). Its input lists, for each agent, the goods it values with their values. Its written analysis, not formalized, gives $O((n+m)\log n)$ operations after the input is read; reading the input takes $O(n+m)$ steps in this form, and a dense $n\times m$ table would take $nm$.

\paragraph{Measurements.} The repository~\cite{Repo} records the following measurements of implementations. None of them is part of a theorem.
\begin{itemize}
\item \texttt{fast} returns the same allocation as a literal transcription of the Lean definitions on $200{,}000$ random instances; on $675$ instances both agree with Lean's own evaluation of \lean{EFX.K3.algo}, every Lean output passes a raw \efxz{} check, and every Lean operation count is below the Lean-checked bound. The largest count divided by $(n+m+1)^4$ falls from $0.38$ at $n=2$ to $0.002$ at $n=30$, against the factor $400$ of the bound.
...
This section describes the Lean~4 formalization~\cite{Repo}: what it covers, what a reader must trust, which axioms it uses, how to read its main statements, and what it leaves out. The further checks that the repository runs on it (a replay checker, an independently written statement, a second proof and a second kernel) are in Appendix~\ref{app:audit}; Appendix~\ref{app:lean} maps every result of this paper to its Lean names, and Appendix~\ref{app:spec} maps every object of Section~\ref{sec:algo} to its Lean definition.
...
\item \emph{Theorem~\ref{thm:algo}}: the algorithm, a computable function (\lean{EFX.K3.algo}); its correctness (\lean{EFX.K3.algo_efx0}); and its operation bounds, for the counted program whose value is the algorithm (\lean{EFX.K3.algoC_cost}, and the finer Theorem~\ref{thm:fine}, \lean{EFX.K3.algoC_cost_fine}).
\item \emph{Section~\ref{sec:correct}} in full: the invariants of Phase~1, the validity of the upgrades, soundness, the owner criterion, Lemmas~\ref{lem:r}--\ref{lem:count} and the remark after Lemma~\ref{lem:r} that $r$ lies in the last block, Theorems~\ref{thm:A}, \ref{thm:B} and~\ref{thm:C}, Propositions~\ref{prop:O} and~\ref{prop:size}, and the example of Remark~\ref{rem:repeat}. \LBp{} is formalized both as a relation over all its runs and as a computable function, the one K3ALG uses.
\item \emph{Section~\ref{sec:prelim}}: peeling (rules R1 and R2, and the general Lemma~\ref{lem:peel}); Lemmas~\ref{lem:R1fail}, \ref{lem:junk} and~\ref{lem:L12}; and Corollary~\ref{cor:k2}, for natural-number values, for every run of serial dictatorship. Also the reduction to cores of Section~\ref{sec:cert}.
\item \emph{Corollaries~\ref{cor:rat} and~\ref{cor:real}}: K3ALG on nonnegative rationals, scaled to natural numbers (\lean{EFX.K3.algoRat_efx0}), and on nonnegative values in any linearly ordered cancellative commutative monoid, in the comparison model, with its operation count (\lean{EFX.K3.algoOrd_efx0}, \lean{EFX.K3.algoOrdC_cost}).
\item \emph{Examples} (Appendix~\ref{app:examples}), checked by \texttt{decide}: four runs of K3ALG, among them a peel followed by the owner $r$, a rotation with the owner $k^*$, and the run of Remark~\ref{rem:repeat}; a run of K3ALG on integer values through the oracle of Corollary~\ref{cor:real}; and a run of serial dictatorship.
\end{itemize}
...
The formalization uses core Lean~4 (v4.34.0), with no packages. The word \texttt{sorry} appears nowhere, and the build is free of warnings. Every declaration of the library depends only on the three standard axioms \texttt{propext}, \texttt{Classical.choice} and \texttt{Quot.sound}; this excludes unfinished proofs, \texttt{native\_decide} (which would trust the compiler) and new axioms. The repository's check script verifies this for all declarations on every pull request, and every main result also carries a \verb|#print axioms| certificate at the end of its file (Appendix~\ref{app:audit} lists the script's further checks).
...
\begin{alltt}\small
theorem EFX.K3.algoC_cost_fine'' (I : Inst) (hn : 0 < I.n) :
    (algoC I hn).cost \(\le\) 270 * (I.n ^ 4 + I.n ^ 2 * I.m)
theorem EFX.K3.algoRat_efx0 (I : OInst Rat) (hn : 0 < I.n)
    (hv : \(\forall\) i g, 0 \(\le\) I.v i g)
    (h : \(\forall\) i, I.numRelevant i \(\le\) 3) : I.EFX0 (algoRat I hn)
theorem EFX.K3.algoOrd_efx0 \{V : Type\} [OrderedValue V]
    \{le : V \(\to\) V \(\to\) Bool\} (hle : \(\forall\) x y, le x y = true \(\leftrightarrow\) x \(\le\) y)
    (I : OInst V) (hn : 0 < I.n) (hv : \(\forall\) i g, 0 \(\le\) I.v i g)
    (h : \(\forall\) i, I.numRelevant i \(\le\) 3) : I.EFX0 (algoOrd le I hn)
theorem EFX.K3.algoOrdC_cost \{V : Type\} [OrderedValue V]
    (le : V \(\to\) V \(\to\) Bool) (I : OInst V) (hn : 0 < I.n) :
    (algoOrdC le I hn).cost \(\le\) I.n * (I.m + 12) + 10 * (I.n * I.m)
      + 971 * I.n + 400 * (I.n + I.m + 1) ^ 4
theorem EFX.Inst.sdRun_efx0 (I : Inst) (h : \(\forall\) i, numRelevant I i \(\le\) 2)
    \{order : List (Fin I.n)\} (hord : order.Nodup) (hall : \(\forall\) i, i \(\in\) order)
    \{X : I.Alloc\} (hX : SDRun I.v order (List.finRange I.m) X) : I.EFX0 X
\end{alltt}
These are Theorem~\ref{thm:fine}, Corollaries~\ref{cor:rat} and~\ref{cor:real}, and Corollary~\ref{cor:k2}. Their statements use definitions beyond the trusted base, which a reader must compare with the text: \lean{EFX.K3.algoRat} is \lean{EFX.K3.algo} on \lean{EFX.ratScale}, the instance whose values are agent $i$'s values times the product of their denominators, as natural numbers; \lean{EFX.K3.algoOrd} is the value of the counted program \lean{EFX.K3.algoOrdC}, which receives the oracle \texttt{le}, computes the surrogate \lean{EFX.K3.surrogate} and runs \lean{EFX.K3.algoC} on it, and \texttt{hle} says that the oracle is correct; and \lean{EFX.SDRun} is the set of runs of serial dictatorship in the order \texttt{order} (each agent once, by \texttt{hord} and \texttt{hall}), with any favourite at each step.

\paragraph{Intermediate statements.} The Lean statements of Section~\ref{sec:correct}'s results are stated over the formalization's own definitions of the state of \LBp{} (picks, blocks, junk, slots, exposure, and so on, in \texttt{PreAlloc.lean}, \texttt{Blocks.lean}, \texttt{OwnerR.lean}, \texttt{Rotation.lean} and \texttt{LBPlus.lean}). To know that such a statement is the theorem of this paper, a reader must compare those definitions with Section~\ref{sec:algo}; Appendix~\ref{app:spec} lists, for each object of Section~\ref{sec:algo}, the Lean definition that formalizes it. This is not needed to trust Theorems~\ref{thm:target}--\ref{thm:algo}, whose statements use only the trusted base. For instance, \lean{EFX.LB.lbPlusRun_sound} quantifies over all runs of \LBp{}: every order of Phase~1 with R1 priority, every upgrade order, every need chain and every completion satisfying (OC).

\subsection{What is not formalized}\label{sec:notformal}

\begin{itemize}
\item Corollary~\ref{cor:bits}, the bit complexity, and the analysis of the faster implementation \texttt{fast} (Section~\ref{sec:finer}).
\item The remarks of Section~\ref{sec:ratreal} that K3ALG run directly on the rational or real values makes the same decisions as on the scaled values or on $w$, and the cost of computing the scaling of Corollary~\ref{cor:rat}.
\item Corollary~\ref{cor:k2} for values other than natural numbers (its form for an ordered type).
\item Proposition~\ref{prop:neg}, the limits of the shape, and Lemma~\ref{lem:threat}.
...
This section explains what exhaustive computation over small instances establishes, independently of the proof of Section~\ref{sec:correct}, and shows, by small counterexamples, that the shape of Theorem~\ref{thm:D} cannot be improved in the obvious ways. The sizes of the runs, the tests, the certificate format and how to reproduce the computations are in Appendix~\ref{app:cert}.
...
\end{itemize}
Counting incidences, a core has $m\le2n$ goods. Cores with $m=2n$ are solved directly: an agent holding two of its own three goods is safe, by balance (\lean{EFX.Inst.safe_of_two_own}), and in a connected core with $m=2n$ every agent can be given its private good and one shared good (this second fact is written, not formalized: its proof is recorded in the repository).

\subsection{What the computation establishes}\label{sec:certn7}

For every number of agents $n\le7$ and every number of goods $m$, the connected cores were listed up to isomorphism, and for every core and every one of its $6^n$ strict ranking profiles an \efxz{} allocation was found and stored. A separately written checker re-checks the stored allocations from the raw \efxz{} definition, and the completeness of each list of cores was checked by a second, independent method. Appendix~\ref{app:cert} gives the sizes of the runs, the methods, the certificate format and how to reproduce them. With Proposition~\ref{prop:core}, the components argument and the cores with $m=2n$, the computation establishes the following.
\begin{itemize}
\item \emph{Theorem~\ref{thm:target} for $n\le7$}, verified by exhaustive computation independently of Section~\ref{sec:correct}: neither the search nor the checker uses \LBp{} or its proof.
\item \emph{Theorem~\ref{thm:D} for every core with $n\le6$, every connected core with $n=7$, and every connected core with $n=8$ and $m\ge13$}, verified by exhaustive computation in the same way. The shape of Theorem~\ref{thm:D} was first observed in these computations.
\end{itemize}
It establishes nothing for larger $n$. Some certificates were produced by an earlier construction, LB; what the runs show about LB itself is stated in Appendix~\ref{app:cert}.
...
\begin{proposition}[limits of the shape; theorem, proof in this paper]\label{prop:neg}
...
Every instance in which each agent positively values at most three goods has a complete \efxz{} allocation, and one can be computed in polynomial time by K3ALG: peel agents by rule R1, run a serial dictatorship that serves agents who lost goods first, put the leftover goods into free slots, and give the surplus to one owner, after at most one rotation. When every agent values exactly three goods and is balanced, there is a complete \efxz{} allocation in which all but at most one agent receive at most two goods. The correctness of K3ALG and its operation bounds, in an explicit unit-cost model, are machine-checked on the program itself.
...
\noindent In Lean, \lean{EFX.l12} proves this for values in any \lean{EFX.OrderedValue}, using all $31$ permutations of these representatives, checked by \texttt{decide}, instead of sorting; agents with one or two relevant goods are treated as three slots, one or two of them phantom. The choice of a pattern needs at most three comparisons to sort $a$, $b$, $c$, two for the ties and one of $a$ with $b+c$; the formalized program of Corollary~\ref{cor:real} asks twelve comparisons instead of sorting (Section~\ref{sec:ratreal}).

\section{The Cost of Each Step}\label{app:cost}

The machine-checked bound of Theorem~\ref{thm:time}, step by step, with $N=n+m+1$; the proof raises each term to a multiple of $N^4$ and sums. The last column is the order of each step in the finer count of Theorem~\ref{thm:fine}, which Lean also checks stage by stage (\texttt{K3CostFine.lean}), with a bound $n$ on the lists of agents and $m$ on the lists of goods.
...
\toprule
Step & Lean bound & finer (Thm.~\ref{thm:fine}) \\
\midrule
...
upgrades: at most $n$ rounds $\times$ $n$ agents $\times$ ($b_k\in\NA$: $n$ agents, membership in $U$; $c_k\in J$) & $36N^4$ & $n^4+17n^3+n^2m+O(n^2+nm)$ \\
...
\paragraph{The examples in the Lean library.} The library itself checks three runs of K3ALG by \texttt{decide}, carried over to \lean{EFX.K3.algo} by \lean{EFX.K3.algo_eq_spec}. In \lean{EFX.K3.Examples.peelOwner_algo}, agent~0 values $g_2,g_4,g_5$ at $84,73,54$, agent~1 values only $g_1$, at $87$, and agent~2 values $g_4,g_2,g_3$ at $92,83,44$. Agent~1 is peeled with $g_1$; agents $0$ and $2$ form one block, picking $g_2$ and $g_4$; the junk $\{g_0,g_3,g_5\}$ exceeds the two slots; the owner is $r=2$, agent~0 is exposed with $\pi_0=\{g_5\}$, and the output is $X_0=\{g_2,g_5\}$, $X_1=\{g_1\}$, $X_2=\{g_4,g_0,g_3\}$. In \lean{EFX.K3.Examples.rotOwner_algo}, agent~0 values $g_1,g_2,g_0$ at $5,4,2$, and agents $1$ and $2$ both value $g_1,g_3,g_2$ at $3,3,2$ (so $a=g_1$, $b=g_3$, $c=g_2$, the tie broken by index). Agents $0,1,2$ form one block and pick $g_1,g_3,g_2$; agents $0$ and $1$ are frozen; $r=2$ fails, since agent~0 is exposed and no slot is free for the others; the rotation along $0\to1\to2$ gives agent~0 the goods $g_2$ and $g_0$ and makes it the owner, and the output is $X_0=\{g_0,g_2,g_5\}$, $X_1=\{g_1\}$, $X_2=\{g_3,g_4\}$. The trace script recomputes these runs as well. The library also checks the run of Remark~\ref{rem:repeat} (\lean{EFX.K3.Examples.repeatedGood_state}: $\HitSet(E_3)=(g_0,g_0)$; \lean{EFX.K3.Examples.repeatedGood_algo}: the owner receives $4=\omega+3$ goods and the output is \efxz{}); the instance of \lean{EFX.K3.Examples.peelOwner_algo} with integer values, run through the oracle of Corollary~\ref{cor:real} (\lean{EFX.K3.Examples.peelOwnerZ_algoOrd}: agent~0's surrogate values are $4,3,2$, and the output is the same); and a run of serial dictatorship on a $2$-relevant instance (\lean{EFX.K3.Examples.twoRel_serialDict}).

\section{The Formalization: Audit Layers and Lean Names}\label{app:audit}

This appendix collects the checks that the repository runs on the formalization beyond those of Section~\ref{sec:lean}, and the Lean names of every result.

\subsection{Checks run by the repository}

The repository's check script, \file{lean/check.sh}, runs on every pull request. Besides the axiom check of Section~\ref{sec:axioms}, it requires a build free of warnings and of \texttt{sorry}, refuses debug options, metaprogramming and unsafe code, and replays the whole library in Lean's replay checker, \texttt{leanchecker} with the option \texttt{-{}-fresh}, which re-checks every declaration in a fresh kernel.
...
\subsection{Lean names}\label{app:lean}
...
Thm.~\ref{thm:fine} & \lean{EFX.K3.algoC_cost_fine}, \lean{EFX.K3.algoC_cost_fine'}, \lean{EFX.K3.algoC_cost_fine''}, \lean{EFX.K3.lbUpC_cost_fine}, \lean{EFX.K3.lbPlusC_cost_fine}, \lean{EFX.K3.reduceC_cost_fine} (\texttt{K3CostFine}) \\
Cor.~\ref{cor:rat} & \lean{EFX.K3.algoRat_efx0}, \lean{EFX.ratScale_val}, \lean{EFX.ratScale_agree}, \lean{EFX.ratScale_numRelevant}, \lean{EFX.ratScale_efx0_iff} (\texttt{K3Extras}) \\
Cor.~\ref{cor:real} & \lean{EFX.K3.algoOrd_efx0}, \lean{EFX.K3.algoOrd_eq}, \lean{EFX.K3.agree_surrogate}, \lean{EFX.K3.surrogateC_cost}, \lean{EFX.K3.algoOrdC_cost} (\texttt{K3Real}); \lean{EFX.K3.algoOrdC_cost_fine} (\texttt{K3CostFine}) \\
...
Cor.~\ref{cor:k2} & \lean{EFX.Inst.sdRun_efx0}, \lean{EFX.Inst.serialDict_efx0}, \lean{EFX.sdRun_efx0}, \lean{EFX.serialDict_run} (\texttt{K3Extras}); existence: \lean{EFX.exists_efx0_of_count} (\texttt{Bridge}) \\
...
$r$ in the last block (Sect.~\ref{sec:rexp}) & \lean{EFX.LB.lastOut_lastBlock}, \lean{EFX.LB.blk_le_lastOut} (\texttt{K3Extras}) \\
...
Prop.~\ref{prop:size} & \lean{EFX.LB.largeBundle_size}, \lean{EFX.LB.omega_eq}, \lean{EFX.LB.numFrozen_eq_numNA}, \lean{EFX.LB.Completion.owner_length_ge}, \lean{EFX.LB.Completion.owner_length_eq}, \lean{EFX.LB.BadCase.omega_le} (\texttt{K3Extras}) \\
Rem.~\ref{rem:repeat} & \lean{EFX.LB.complete_owner_length}, \lean{EFX.K3.Examples.repeatedGood_state}, \lean{EFX.K3.Examples.repeatedGood_algo} (\texttt{K3Extras}) \\
...
Examples (App.~\ref{app:examples}) & \lean{EFX.K3.Examples.peelOwner_algo}, \lean{EFX.K3.Examples.rotOwner_algo}, \lean{EFX.K3.Examples.rot_algo} (\texttt{K3Examples}); \lean{EFX.K3.Examples.peelOwnerZ_algoOrd} (\texttt{K3Real}); \lean{EFX.K3.Examples.twoRel_serialDict} (\texttt{K3Extras}) \\
...
\section{Specification Correspondence}\label{app:spec}

The Lean statements of Section~\ref{sec:correct}'s results use the formalization's own definitions of the state of Stage L. Tables~\ref{tab:spec} and~\ref{tab:spec2} map each object of Section~\ref{sec:algo}, and of the corollaries, to the Lean definition that formalizes it, so that a reader can compare them. Names are in the namespace \texttt{EFX.LB} unless they are written in full, starting with \texttt{EFX.}, and files are in \file{lean/EFX/}. The Lean state is a ranking profile \texttt{P} (the goods $a_i$, $b_i$, $c_i$ of each agent), the lists of agents and goods, the picks \texttt{Y}, a function from agents to optional goods, and the list \texttt{up} of upgraded agents; most objects below are functions of this state. Where no single Lean definition corresponds to an object, the table says so.

\begin{table}[tbp]
\caption{Objects of Section~\ref{sec:algo} and their Lean definitions.}\label{tab:spec}
\centering\small
\setlength{\tabcolsep}{4pt}\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.22\textwidth}>{\raggedright\arraybackslash}p{0.54\textwidth}>{\raggedright\arraybackslash}p{0.18\textwidth}@{}}
\toprule
Object & Lean definition & File \\
\midrule
rankings $a_i\succ_i b_i\succ_i c_i$ & \lean{Profile} (fields \texttt{a}, \texttt{b}, \texttt{c}), \lean{Profile.rank}; computed by \lean{EFX.K3.profileOf} & \texttt{LBSound}, \texttt{K3Algo} \\
Stage R, rule R1 & \lean{EFX.K3.r1Step}, \lean{EFX.K3.findR1}, \lean{EFX.K3.reduce} & \texttt{K3Algo} \\
R1 priority & \lean{R1Prio} (a property of an order); K3ALG's order \lean{r1Order} & \texttt{Blocks}, \texttt{CorollaryD} \\
picks $Y$ & \lean{phase1} (Phase~1 in a given order) & \texttt{LBRun} \\
blocks, leaders & \lean{blkAux} (block number, the insertion steps up to the agent's turn), \lean{leadB} (leader test); what the proofs use of them: \lean{Run} & \texttt{Blocks} \\
upgrades, $U$ & \lean{upgrades} (the loop), \lean{canUp} (its test), \lean{lbUp} (K3ALG's $U$), \lean{UpFinal} (any order) & \texttt{LBRun}, \texttt{Blocks} \\
junk $J$ & \lean{junkList}; $J_0$: \lean{junk0} & \texttt{PreAlloc}, \texttt{Blocks} \\
needs $N_i$ & no definition of the set; ``$i$ needs $g$'' is \lean{Profile.Prefers} & \texttt{LBSound} \\
$\NA$ & \lean{Profile.NA} (a proposition), \lean{naB} (a Boolean) & \texttt{LBSound}, \texttt{LBRun} \\
frozen & \lean{frozenB} & \texttt{LBRun} \\
terminal & \lean{IsTerm} & \texttt{OwnerR} \\
$\capa$, $S$ & \lean{cap}, \lean{slotSum}; the slots of all agents but the owner: \lean{slotsExcept} & \texttt{LBRun}, \texttt{PreAlloc} \\
$\omega$, $|\NA|$, $|F|$ & $\omega$ is the length of \lean{junkList} minus \lean{slotSum}, as an integer; \lean{numNA} ($\NA$ counted over the goods), \lean{numFrozen} & \texttt{K3Extras} \\
base & \lean{InBase} (membership in the base) & \texttt{PreAlloc} \\
valid; (UT) & \lean{Valid}; the field \texttt{ut} of \lean{State} & \texttt{PreAlloc}, \texttt{OwnerR} \\
\bottomrule
\end{tabular}
\end{table}

\begin{table}[tbp]
\caption{Table~\ref{tab:spec}, continued; the last three rows are objects of Corollaries~\ref{cor:k2}, \ref{cor:rat} and~\ref{cor:real}.}\label{tab:spec2}
\centering\small
\setlength{\tabcolsep}{4pt}\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.22\textwidth}>{\raggedright\arraybackslash}p{0.54\textwidth}>{\raggedright\arraybackslash}p{0.18\textwidth}@{}}
\toprule
Object & Lean definition & File \\
\midrule
completion, (OC) & \lean{Completion} (every completion; its field \texttt{oc} is (OC)); K3ALG's completion \lean{complete}, which places the junk by \lean{fill} & \texttt{PreAlloc}, \texttt{LBRun} \\
owner $o$ & no single definition: the argument \texttt{o} of \lean{Completion} and \lean{complete} (an optional agent) & \texttt{PreAlloc} \\
exposed $E_o$ & \lean{Exposed}, \lean{exposedL} & \texttt{OwnerR} \\
$\pi_x$ & no single definition: one of its goods is \lean{pickOne}, and two meeting sets are found by \lean{meet} & \texttt{OwnerR} \\
$\HitSet$ & \lean{hitSet} & \texttt{OwnerR} \\
valid owner & \lean{ValidOwner}, with \lean{OwnerOK} & \texttt{OwnerR} \\
candidate $r$; $k^*$ & \lean{lastOut}; \lean{kstar} & \texttt{OwnerR} \\
need chain & \lean{IsChain}, \lean{NeedChain}; K3ALG's scan: \lean{chainFrom} with \lean{isNext} & \texttt{OwnerR}, \texttt{LBPlus} \\
bad case & \lean{BadCase} & \texttt{Rotation} \\
rotation & \lean{rotPicks} (with \lean{rotY}); the upgraded list becomes \texttt{k :: up} & \texttt{Rotation} \\
\LBp{} & \lean{lbPlus} (K3ALG's run), \lean{LBPlusOut} and \lean{LBPlusRun} (all runs) & \texttt{LBPlus} \\
K3ALG & \lean{EFX.K3.algoSpec} (specification), \lean{EFX.K3.algoC} (counted program), \lean{EFX.K3.algo} (its value) & \texttt{K3Algo}, \texttt{K3Cost} \\
serial dictatorship (Cor.~\ref{cor:k2}) & \lean{EFX.SDRun} (all runs), \lean{EFX.Inst.serialDict} (one run, with \lean{EFX.favorite}) & \texttt{K3Extras} \\
scaling (Cor.~\ref{cor:rat}) & \lean{EFX.ratScale}, with \lean{EFX.denProd} (the product of the denominators); \lean{EFX.K3.algoRat} & \texttt{K3Extras} \\
oracle, $w$ (Cor.~\ref{cor:real}) & \lean{EFX.K3.askC} (one call), \lean{EFX.K3.surrogate} and \lean{EFX.K3.surrogateC} ($w$), \lean{EFX.K3.algoOrd} and \lean{EFX.K3.algoOrdC} & \texttt{K3Real} \\
\bottomrule
\end{tabular}
\end{table}

\section{Certification: Runs, Tests and Reproducibility}\label{app:cert}

This appendix gives the details behind Section~\ref{sec:cert}: the sizes of the runs and the methods, the tests of \LBp{} and K3ALG, and what is needed to reproduce them.

\subsection{Runs and methods}
...
\subsection{Tests of \LBp{} and K3ALG (measurement)}\label{sec:tests}

A literal implementation of \LBp{} was run $5{,}844{,}263{,}904$ times: on every core with $n\le6$ under every ranking profile and every sequence of insertion choices, with random choices on larger cores, and on random instances that are not cores. It asserts every lemma of Section~\ref{sec:correct} along the way and checks every output against the raw definition: $1{,}327{,}101$ runs needed the rotation, and there were $0$ failures. An independent Python implementation gives identical tallies for $n\le5$. K3ALG itself returned an \efxz{} allocation on all $152{,}071{,}632$ ranking profiles of all cores with $n\le6$. These are measurements of the implementations, not part of the proof.

\subsection{Reproducibility}

\paragraph{Version.} The files named here are those of commit \texttt{c6d1871} of the repository~\cite{Repo}, the commit cited in the bibliography; the certificates, scripts and Lean sources are unchanged between that commit and the version of this paper. The repository has no release tag.

\paragraph{Scripts.} Enumeration of the connected cores: \file{src/cores_nauty.py}, with nauty's \texttt{genbg}, cross-checked against the independent generator of \file{src/frontier.py}; orbit counting: \file{tools/check_enum.py}. Search with a SAT solver and counterexample-guided refinement: \file{src/frontier.py}, driven by \file{src/run7.py} for $n\ge7$. Construction LB: \file{src/construct.c} with \file{src/construct_run.py}. The SAT-free certificate checker: \file{tools/check_certs.py}. Tests: \file{src/lbplus.c} and the independent \file{src/lbplus.py} for \LBp{}, and \file{k3/k3algo.py} for K3ALG.

\paragraph{Certificate format.} Each certificate file, \file{results/certs_*.json.gz}, is a gzip-compressed JSON list with one record per connected core: the numbers \texttt{n} and \texttt{m}; \texttt{pi}, the number of private goods; \texttt{sets}, the three relevant goods of each agent; \texttt{mode}, a label (for the SAT runs, the shape of allocation searched for); and \texttt{allocations}, a list of allocations, each giving the agent of every good, which together must cover all $6^n$ ranking profiles. The checker decides, for every stored allocation, in which profiles it is \efxz{}, from the raw definition under three balanced realizations of the rankings, and checks that every profile is covered; with \texttt{-{}-require-d} it also checks that every stored allocation has at most one bundle of more than two goods.

\paragraph{Machines and running times.} The logs in \file{results/} record the commands, their dates, the number of CPUs (four, or two for some re-checks) and some wall-clock times. For instance, the SAT search over the $3{,}103$ connected cores with $n=7$ and $m=11$ took $684$\,s on four CPUs (\file{results/frontier7_11.log}); K3ALG on every ranking profile of every core with $n\le6$ took $4{,}469$\,s on four CPUs (\file{results/k3_certs_2_6.log}); and construction LB on the connected cores with $n=8$ and $m\ge13$ took about $7$\,h on four CPUs (the repository's ledger). The processor model and the memory are not recorded. The measurements of Section~\ref{sec:finer} used Python~3.11.15 (\file{results/k3_timing.log}).

\paragraph{Re-checking.} \texttt{python tools/check\_certs.py} \emph{file} \texttt{-{}-require-d} re-checks one certificate file. The repository's continuous integration re-checks some of the certificates and the orbit counts on every pull request; the larger files were re-checked locally, as the logs record.
