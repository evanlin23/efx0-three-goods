...
\begin{abstract}
An allocation of indivisible goods is \efxz{} if no agent envies another agent's bundle after the removal of any single good from it, even a good that the envious agent values at zero. We prove that every instance with nonnegative additive valuations in which each agent positively values at most three goods has a complete \efxz{} allocation, for any number of agents. The proof is an algorithm, K3ALG, that runs in polynomial time and returns such an allocation; its correctness and a bound of $400(n+m+1)^4$ counted operations are machine-checked in Lean~4 on the program itself, for natural-number values (rational values reduce to these by scaling). K3ALG removes agents that value at most two remaining goods or have a dominant good, runs a serial dictatorship on the rest, places the unpicked goods into free slots and one large bundle, and, when the natural owner of that bundle fails, rotates picks once along a chain of agents. The correctness proof also shows that when every agent values exactly three goods and none of them more than the other two together, some \efxz{} allocation has at most one bundle of more than two goods. Both existence results are machine-checked as well, in core Lean with the standard axioms only, for values in any linearly ordered cancellative commutative monoid, a class that contains the nonnegative reals. Bundles of at most two goods do not always suffice, even with fewer than $2n$ goods.
\keywords{Fair division \and Indivisible goods \and EFX \and Serial dictatorship \and Formal verification \and Lean}
\end{abstract}

\section{Introduction}

Envy-freeness up to any good is a central relaxation of envy-freeness for indivisible goods~\cite{AmanatidisBFV22}. Let agents $N=[n]$ share goods $M=[m]$, with additive valuations $v_i(S)=\sum_{g\in S}v_i(g)$, $v_i(g)\ge0$. An allocation $X=(X_1,\dots,X_n)$, a partition of $M$, is \emph{\efxz{}} if
\[
  v_i(X_i)\ \ge\ v_i(X_j\setminus\{h\})\qquad\text{for all } i\ne j \text{ and all } h\in X_j,
\]
and \emph{EFX} if this is required only for goods $h$ with $v_i(h)>0$; the stronger notion is usually called \efxz{}~\cite{AmanatidisBFV22}. The difference matters when agents value few goods: if $X_j$ contains a good worthless to $i$, then \efxz{} requires $v_i(X_i)\ge v_i(X_j)$, so goods that nobody values cannot be handed to an arbitrary agent.

Complete EFX allocations are known to exist for additive valuations only in special cases, among them three agents~\cite{ChaudhuryGM20}, $m\le n+3$~\cite{Mahara21}, four agents with at most nine goods~\cite{AlkassarFM26}, when every good is valued by at most two agents~\cite{AfshinmehrAMMS26} (special cases earlier~\cite{ChristodoulouFKS23,SgouritsaS25,KavianiSS24}), and on hypergraphs of girth at least four~\cite{LianeasSS26}; these are \efxz{} results. For monotone valuations EFX can fail~\cite{AkramiMMSW26,MackenzieS26}. Call an instance \emph{$k$-relevant} if every agent positively values at most $k$ goods. For $k\le2$, serial dictatorship gives an \efxz{} allocation~\cite{MrdEfx}. For $k\le4$, Viswanathan and Mehta~\cite{ViswanathanM24} announce ordinary EFX, with a proof sketch; their algorithm gives the goods nobody values to an arbitrary agent, which can break \efxz{}. We ask whether every $3$-relevant instance has a complete \efxz{} allocation. A counterexample would refute the additive EFX conjecture, since perturbing all values by a small $\varepsilon>0$ (compare~\cite[Lemma~1]{ChaudhuryGM20}) turns it into a positive instance without EFX allocations; but the perturbation destroys the relevance structure, and we know of no result that implies a positive answer (see the literature search in~\cite{Repo}).
...
On every instance with $n\ge1$ agents and natural-number values in which every agent positively values at most three goods, algorithm K3ALG (Section~\ref{sec:algo}) returns an \efxz{} allocation. On every instance with $n\ge1$ agents it performs at most $400(n+m+1)^4$ counted operations.
\end{theorem}

\noindent Scaling each agent's values preserves \efxz{}, so Theorem~\ref{thm:algo} covers rational values (not formalized).

\paragraph{Contributions.} (i)~Existence over the reals (Theorem~\ref{thm:target}), with the structure of Theorem~\ref{thm:D}. (ii)~A polynomial-time algorithm, K3ALG, with correctness and operation bound machine-checked on the program itself (Theorem~\ref{thm:algo}); its correctness proof (Section~\ref{sec:correct}) also gives (i). (iii)~A formalization in core Lean~4 with the standard axioms only (Section~\ref{sec:lean}). (iv)~Limits: even in cores, bundles of at most two goods do not always suffice (smallest: three agents, five goods), nor does one bundle of three goods (Proposition~\ref{prop:neg}).

\paragraph{Status of the claims.} The work was done in a public repository~\cite{Repo} whose ledger gives every claim a status: PROVED (complete written proof), CERTIFIED (exhaustive computation over a provably complete set, with independently checked certificates), EVIDENCE, CONJECTURE or REFUTED. Every theorem, lemma and proposition below is PROVED; results marked \emph{written} are not ledger claims, and Section~\ref{sec:cert} labels CERTIFIED and EVIDENCE results. Two independent reviews of the proof of Theorems~\ref{thm:target} and~\ref{thm:D} found no error.
...
\noindent Such comparisons depend only on the ties among $a_i,b_i,c_i$ and the sign of $v_i(a_i)-v_i(b_i)-v_i(c_i)$ (Appendix~\ref{app:prelim}).

\section{Algorithm K3ALG}\label{sec:algo}

K3ALG has two stages: \emph{Stage R} peels agents by rule R1, and \emph{Stage L} runs construction \LBp{} on the remaining agents $A$ and goods $M$; each of these agents values exactly three goods and is strictly balanced. Stage L first runs a serial dictatorship with \emph{R1 priority}: the next agent has at most two of its goods left if such an agent exists (an \emph{R1 step}), and otherwise it finds all three available and takes its top (an \emph{insertion step}). Let $G$ be the set of goods not yet picked (initially $M$). Each agent $i$ takes its $\succ_i$-favourite remaining good as its \emph{pick} $Y_i$, or no pick ($Y_i=\none$). A \emph{block} is an insertion step with the R1 steps that follow it, and its inserted agent is its \emph{leader}. The upgrades then give some agents $u$ with $Y_u=b_u$ also $c_u$; these agents form $U$. The \emph{junk} $J$ is the set of goods that are neither picks nor such goods $c_u$. For $i\notin U$ the \emph{needs} are $N_i=\{g\in R_i\st g\succ_i Y_i\}$ ($N_i=R_i$ if $Y_i=\none$), and $\NA=\bigcup_{i\notin U}N_i$ is the set of goods \emph{needed alone}. An agent $i\notin U$ is \emph{frozen} if $Y_i\in\NA$, and a \emph{terminal} otherwise. The \emph{slots} are $\capa(i)=0$ for upgraded and frozen agents, and $1$ or $2$ for a terminal with or without a pick; $S=\sum_i\capa(i)$. The \emph{base} of $i$ is $\{Y_i\}$ (or $\emptyset$) for $i\notin U$ and $\{b_i,c_i\}$ for $i\in U$. A \emph{completion} with \emph{owner} $o$ (a terminal or upgraded agent) gives every agent its base, at most $\capa(i)$ junk goods to every terminal $i\ne o$, and the rest of the junk to $o$; it satisfies the \emph{owner constraint} (OC) if no agent $x\notin U$, $x\ne o$, with $Y_x=a_x$ has $\{b_x,c_x\}\subseteq X_o$. The agents \emph{exposed} for $o$ are $E_o=\{x\notin U,\ x\ne o\st Y_x=a_x,\ \{b_x,c_x\}\subseteq J\cup\base(o)\}$, and $\pi_x=\{b_x,c_x\}\cap J$. A \emph{need chain} from a frozen agent $x$ is a sequence $x=x_0,\dots,x_t$ of distinct agents in which each $x_{s+1}\notin U$ needs $Y_{x_s}$, $x_0,\dots,x_{t-1}$ are frozen, and $x_t$ is a terminal.

\medskip
\noindent\rule{\textwidth}{0.5pt}\par
\noindent\textbf{Algorithm K3ALG} (as formalized in \lean{EFX.K3.algoC}). Input: $n\ge1$ agents, $m$ goods, values $v_i(g)\in\mathbb{N}$; $A$ and $M$ start as all agents and all goods. Lists are in index order, and ``first'' means smallest index.
\begin{enumerate}
\item[P1.] If $|A|\le1$, give $M$ to the agent of $A$ and stop. If $M=\emptyset$, stop.
\item[P2.] Find the first $i\in A$ such that, for the first good $p\in M$ of largest $v_i(p)$, $v_i(p)=0$ or $v_i(M\setminus\{p\})\le v_i(p)$; if none, go to L1.
\item[P3.] Remove $i$ from $A$; if $v_i(p)>0$, let $X_i=\{p\}$, $M\leftarrow M\setminus\{p\}$. Go to P1.
\item[L1.] For $i\in A$, sort $i$'s relevant goods in $M$ as $a_i,b_i,c_i$ (ties by index).
\item[L2.] \emph{Phase 1.} Repeatedly, the first unprocessed agent with at most two of its goods unpicked (else the first unprocessed agent) picks the first of $a_i,b_i,c_i$ still unpicked, if any.
\item[L3.] \emph{Upgrades.} $U\leftarrow\emptyset$. While some $k\notin U$ has $Y_k=b_k$, $c_k\in J$ and $b_k\notin\NA$, add the first such $k$ to $U$.
\item[L4.] If $|J|\le S$, return $\textsc{Complete}(\text{no owner},\emptyset)$.
\item[L5.] Let $r$ be the last-processed agent not in $U$, and $H=\textsc{HitSet}(E_r)$. If $|H|\le S-\capa(r)$, return $\textsc{Complete}(r,H)$.
\item[L6.] \emph{Rotation.} Let $k$ be the first agent of $E_r$ in $r$'s block. Scanning the agents processed after $k$ in order, append $j$ to the chain $k,\dots$ if the chain's last agent is frozen, $j\notin U$, and $j$ needs the pick of the chain's last agent. Each chain agent takes its predecessor's pick; $k$ takes $b_k,c_k$ and joins $U$.
\item[L7.] Recompute $J$, $\capa$ and $S$. If $|J|\le S$, return $\textsc{Complete}(\text{no owner},\emptyset)$; otherwise return $\textsc{Complete}(k,\textsc{HitSet}(E_k))$.
\end{enumerate}
$\textsc{HitSet}(E)$: if two agents $x\ne y$ of $E$ share a good of $\pi_x\cap\pi_y$ (the first such $x$, then the first such $y$, then the first such good among $b_x,c_x$), return that good followed by, for each other $z\in E$ in order, $b_z$ if $b_z\in J$ and $c_z$ otherwise; otherwise return that list for all $z\in E$.
$\textsc{Complete}(o,H)$: picks go to their pickers, $c_u$ to each $u\in U$; the junk, $H$ first and then the rest of $J$, fills the slots of the agents other than $o$ in index order; $o$ gets the remaining junk.\par
\noindent\rule{\textwidth}{0.5pt}\par

\section{Correctness}\label{sec:correct}

Stage R is correct by Lemma~\ref{lem:peel}. For Stage L we prove more, for every Phase~1 order with R1 priority, upgrade order and need chain (construction \LBp{}). Phase~1 has four immediate invariants (Appendix~\ref{app:proofs}): (I1)~if $g\in R_i$ and $g\succ_i Y_i$ (or $Y_i=\none$), then $g$ was picked before $i$'s turn, and (B)~by an agent of $i$'s block; (I2)~if $g\in R_i$ is never picked, then $Y_i\succ_i g$; (I3)~an agent that finds all its goods available is a leader. A pair $(Y,U)$ as above is \emph{valid} if (V1)~$J\cap\NA=\emptyset$ and (V2)~$b_u,c_u\notin\NA$ for $u\in U$. After L3 it is valid, by (I2) and because $\NA$ only shrinks (Lemma~\ref{lem:upgrades}).

\begin{theorem}[soundness]\label{thm:sound}
Let $(Y,U)$ be valid. Every completion that satisfies (OC) is \efxz{} for every balanced additive valuation consistent with the rankings, and only the owner's bundle can have more than two goods.
\end{theorem}

\begin{proof}[idea]
A bundle of two or more goods contains no good needed alone, by (V1) and (V2), so it can threaten agent $i$ only through $b_i$ and $c_i$ together, while $i$ holds only $a_i$; with two goods that is harmless, and with more it is the owner's bundle, which (OC) excludes (Appendix~\ref{app:proofs}).\qed
\end{proof}

\begin{lemma}[owner criterion]\label{lem:owner}
Let $(Y,U)$ be valid with $|J|>S$. If no $x\in E_o$ has $\{b_x,c_x\}\subseteq\base(o)$, and some $H\subseteq J$ with $|H|\le S-\capa(o)$ meets $\{b_x,c_x\}$ for every $x\in E_o$, then a completion with owner $o$ satisfies (OC).
\end{lemma}

\noindent (Fill the other slots with $H$ first.) Then $o$ is a \emph{valid owner}. If $|J|\le S$, the completion without owner has all bundles of at most two goods.
...
\begin{proof}[idea]
An exposed agent found all its goods available, so it leads its block (I3). For each exposed $x$ outside $r$'s block, $x$ or the end of a need chain from $x$ is a terminal in $x$'s block (B); these terminals are distinct and differ from $r$, so $S-\capa(r)\ge|E_r|-1$, or $\ge|E_r|$ without $k^*$. One junk good per $\pi_x$, with a shared good where two meet, then fits unless the bad case holds (Appendix~\ref{app:proofs}).\qed
\end{proof}

\begin{theorem}[rotation]\label{thm:B}
In the bad case, rotate along any need chain $k^*=x_0,\dots,x_t=r$: each $x_s$ ($s\ge1$) takes $Y_{x_{s-1}}$, and $k^*$ takes $b_{k^*},c_{k^*}$ and joins $U$. The result is valid, and if its junk exceeds its slots, then $k^*$ is a valid owner.
\end{theorem}

\begin{proof}[idea]
Chain agents get goods they needed, so $\NA$ shrinks, and $b_{k^*},c_{k^*}\in J\cup\{Y_r\}$ are free. Agents exposed for $k^*$ afterwards were exposed for $r$, disjointness keeps their pairs out of $\{b_{k^*},c_{k^*}\}$, and the terminals found for them lie outside $r$'s block and keep their slots (Appendix~\ref{app:proofs}, with Example~\ref{ex:rot}).\qed
\end{proof}

\noindent \LBp{} is Stage L with any Phase~1 order with R1 priority and upgrade order, owner $r$ if valid, and otherwise a rotation along any need chain from $k^*$ to $r$ and owner $k^*$.

\begin{theorem}[\LBp{} never fails]\label{thm:C}
If every agent values exactly three goods and is balanced, then for all these choices \LBp{} returns an allocation that is \efxz{} for every balanced additive valuation consistent with the rankings, with at most one bundle of more than two goods.
\end{theorem}

\begin{proof}
The pair $(Y,U)$ is valid, and the completion satisfies (OC), by Lemma~\ref{lem:owner} and Theorems~\ref{thm:A} and~\ref{thm:B}. Apply Theorem~\ref{thm:sound}.\qed
\end{proof}

\begin{proof}[of Theorems~\ref{thm:target}--\ref{thm:algo} (correctness)]
L2 has R1 priority, L5 tests $r$ exactly (Proposition~\ref{prop:O}), and L6 builds a need chain from $k^*$ to $r$ in the bad case, so Stage L is a run of \LBp{} (Theorem~\ref{thm:C}, hence Theorem~\ref{thm:D}). With Lemma~\ref{lem:peel} and induction on $n$, K3ALG is correct (Theorem~\ref{thm:algo}); Lemma~\ref{lem:L12} gives Theorem~\ref{thm:target}.\qed
\end{proof}

\section{Running Time}\label{sec:time}

A minimum set of junk goods meeting all sets $\pi_x$ (of at most two goods each) is a minimum vertex-cover problem in general, but the owner test for $r$ needs none.
...
\noindent In Lean, K3ALG is a program \lean{EFX.K3.algoC} in a \emph{cost monad} (a value paired with an operation count), and \lean{EFX.K3.algo} is defined as its value, so the bound of Theorem~\ref{thm:algo} is about the program that computes the allocation. One unit is a list step, a comparison of agents or goods, a read of an input value or of a table, or a comparison or addition of natural numbers of any size (Appendix~\ref{app:algo}). The dominant term is the upgrade loop L3. The finer bound $O(n^4+n^2m)$ is written but not formalized, as is $O((n+m)\log n)$ after reading the input for a Python implementation, which agrees with the Lean program on all instances tested and takes about 1.6\,s with $10^5$ agents (evidence); neither is a ledger claim. K3ALG compares only subset sums of one agent's values, so by Lemma~\ref{lem:L12} it runs verbatim on real values (a written remark; the ledger does not claim it).

\section{The Lean Verification}\label{sec:lean}

The formalization uses core Lean~4 (v4.34.0) and no packages; no file contains \texttt{sorry}, every declaration uses only the three standard axioms (checked by a script), and Lean's replay checker re-checks the library. The trusted base is six short definitions copied from~\cite{MrdEfx} (Appendix~\ref{app:lean}): values $v\colon\mathrm{Fin}\,n\to\mathrm{Fin}\,m\to\mathbb{N}$, an allocation as a map from goods to agents, and \efxz{} with the removed good ranging over \emph{every} good of the other bundle. Theorem~\ref{thm:algo} is (in namespace \lean{EFX.K3}):
\begin{alltt}
def algo (I : Inst) (hn : 0 < I.n) : I.Alloc :=
  (algoC I hn).val
theorem algo_efx0 (I : Inst) (hn : 0 < I.n)
    (h : \(\forall\) i, numRelevant I i \(\le\) 3) : I.EFX0 (algo I hn)
theorem algoC_cost (I : Inst) (hn : 0 < I.n) :
    (algoC I hn).cost \(\le\) 400 * (I.n + I.m + 1) ^ 4
\end{alltt}
\lean{EFX.K3.algo_eq_spec} proves that the counted program computes exactly the specification \lean{EFX.K3.algoSpec}, the composition of the peeling loop, the processing order and \LBp{}. So the program whose cost is bounded is the one proved correct. Theorems~\ref{thm:target} and~\ref{thm:D} are \lean{EFX.target} and \lean{EFX.LB.corollaryD} over $\mathbb{N}$, and \lean{EFX.target_ordered} and \lean{EFX.corollaryD_ordered} over any linearly ordered cancellative commutative monoid via Lemma~\ref{lem:L12} (\lean{EFX.l12}); that $\mathbb{R}_{\ge0}$ is one is the textbook fact, as core Lean has no reals. Every result of Section~\ref{sec:correct} is formalized (Appendix~\ref{app:lean}). An independent restatement of Theorems~\ref{thm:target} and~\ref{thm:D} follows from them, and a second kernel re-checked their natural-number development. Not formalized: the size of the large bundle, the finer running-time bounds, the real-value remark, Lemma~\ref{lem:L5} and Proposition~\ref{prop:neg}.

\section{Certification and Evidence}\label{sec:cert}

Independently of the proof, a SAT-free checker verified from the raw \efxz{} definition that stored allocations cover all $6^n$ ranking profiles (Lemma~\ref{lem:L5}) of every connected core (Appendix~\ref{app:prelim}), as enumerated by nauty's \texttt{genbg} and cross-checked (Appendix~\ref{app:cert}). This certifies \efxz{} for all $3$-relevant instances with $n\le7$, and Theorem~\ref{thm:D} for all cores with $n\le6$ and all connected cores with $n=7$, or $n=8$ and $m\ge13$. As evidence, K3ALG returned an \efxz{} allocation on all $152{,}071{,}632$ ranking profiles of all cores with $n\le6$.

\section{Conclusion and Open Problems}\label{sec:concl}

Every $3$-relevant instance has a complete \efxz{} allocation, and a verified polynomial-time algorithm finds one. Four relevant goods per agent is open, as are the minimum size of the large bundle and the complexity of the owner test for arbitrary owners.
...
\paragraph{Cores.} Agent $i$ is \emph{top-heavy} if $v_i(a_i)\ge v_i(b_i)+v_i(c_i)$. \emph{Rule R2} gives $i$ the set $P$ of its relevant goods that no other agent values, if $|P|\ge2$ and (i) of Lemma~\ref{lem:peel} holds. A \emph{core} is an instance with at least two agents, each valuing exactly three goods and strictly balanced, each with at most one \emph{private} good (one no other agent values), and no good valued by nobody.
...
Let agents $0,1,2$ rank $g_0\succ g_2\succ g_3$, $g_2\succ g_1\succ g_4$, $g_0\succ g_2\succ g_1$, processed in the order $1,0,2$ (a Phase~1 order with R1 priority, though not the one of L2). Agent~1 is inserted and picks $g_2$; agents $0$ and $2$ then pick $g_0$ and $g_1$ in R1 steps. Now $J=\{g_3,g_4\}$, $\NA=\{g_0,g_2\}$, agents $0,1$ are frozen, and $r=2$ has one slot. Giving $r$ both junk goods violates (OC) for agent~1 ($b_1=g_1$, $c_1=g_4$). The rotation along the chain $1\to2$ gives $\{g_0\},\{g_1,g_4\},\{g_2,g_3\}$, which is \efxz{}.
\end{example}
...
\section{Running Time: Cost Table and Evidence}\label{app:algo}

The program is the listing of Section~\ref{sec:algo}, formalized as \lean{EFX.K3.algoC}; see \lean{proofs/k3_algorithm.md} of~\cite{Repo}. In L5 and L6 the program completes with owner $r$ (or without owner) if $k$ (or $r$) does not exist; by Theorem~\ref{thm:A} this never happens.
...
Thm.~\ref{thm:algo} [K3.ALG] & \lean{EFX.K3.algo_efx0}, \lean{EFX.K3.algo_eq_spec} (\texttt{K3Cost}) \\
...
Lem.~\ref{lem:owner} [S2.LB+] & \lean{EFX.LB.complete_some} (\texttt{PreAlloc}) \\
Lem.~\ref{lem:upgrades} [S2.LB+] & \lean{EFX.LB.upFinal_valid} (\texttt{Blocks}) \\
...
Prop.~\ref{prop:O} [K3.OWNER] & \lean{EFX.LB.validOwner_iff}, \lean{EFX.LB.hitSet_fits} (\texttt{OwnerR}) \\
...
Theorem~\ref{thm:D}, connected cores, $n=7$, $11\le m\le13$ & $3{,}103$, $541$ and $37$ cores ($m=11,12,13$); $6^7$ profiles each & R2 \\
