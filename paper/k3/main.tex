...
\begin{abstract}
An allocation of indivisible goods is \efxz{} if no agent envies another agent's bundle after the removal of any single good from it, even a good that the envious agent values at zero. We prove that every instance with nonnegative additive valuations in which each agent positively values at most three goods has a complete \efxz{} allocation, for any number of agents. The proof is an algorithm, K3ALG, that runs in polynomial time and returns such an allocation; its correctness and a bound of $400(n+m+1)^4$ counted operations are machine-checked in Lean~4 on the program itself (for natural-number values, hence rational ones by scaling). K3ALG removes agents that value at most two remaining goods or have a dominant good, runs a serial dictatorship on the rest, places the unpicked goods into free slots and one large bundle, and, when the natural owner of that bundle fails, rotates picks once along a chain of agents. So when every agent values exactly three goods and none of them more than the other two together, some \efxz{} allocation has at most one bundle of more than two goods. Both theorems are machine-checked as well, over the reals, in core Lean with the standard axioms only. Bundles of at most two goods do not always suffice.
...
\noindent Scaling each agent's values preserves \efxz{}, so Theorem~\ref{thm:algo} covers rational values.

\paragraph{Contributions.} (i)~Existence over the reals (Theorem~\ref{thm:target}), with the structure of Theorem~\ref{thm:D}. (ii)~A polynomial-time algorithm, K3ALG, with correctness and operation bound machine-checked on the program itself (Theorem~\ref{thm:algo}); its correctness proof (Section~\ref{sec:correct}) also gives (i). (iii)~A formalization in core Lean~4 with the standard axioms only (Section~\ref{sec:lean}). (iv)~Limits: bundles of at most two goods do not always suffice (smallest counterexample: three agents), nor does one bundle of three goods (Proposition~\ref{prop:neg}).

\paragraph{Status of the claims.} The work was done in a public repository~\cite{Repo} whose ledger gives every claim a status: PROVED (complete written proof), CERTIFIED (exhaustive computation over a provably complete set, with independently checked certificates), EVIDENCE, CONJECTURE or REFUTED. All positive results below are PROVED; two independent reviews of the proof of Theorems~\ref{thm:target} and~\ref{thm:D} found no error.
...
\begin{enumerate}
\item[R1.] If $|A|\le1$, give $M$ to the agent of $A$ and stop. If $M=\emptyset$, stop.
\item[R2.] Find the first $i\in A$ such that, for the first good $p\in M$ of largest $v_i(p)$, $v_i(p)=0$ or $v_i(M\setminus\{p\})\le v_i(p)$; if none, go to L1.
\item[R3.] Remove $i$ from $A$; if $v_i(p)>0$, let $X_i=\{p\}$, $M\leftarrow M\setminus\{p\}$. Go to R1.
...
\end{enumerate}
$\textsc{HitSet}(E)$: for each $z\in E$, $b_z$ if $b_z\in J$ and $c_z$ otherwise; but if two agents $x\ne y$ of $E$ (the first such pair) have a common good in $\pi_x\cap\pi_y$, that good replaces the choices for $x$ and $y$.
...
A minimum set of junk goods meeting all sets $\pi_x$ (of at most two goods each) is a vertex cover in general, but the owner test for $r$ needs none.
...
Every $3$-relevant instance has a complete \efxz{} allocation, and a verified polynomial-time algorithm finds one. Four relevant goods per agent is open (the repository has work in progress on it, about which we claim nothing), as are the minimum size of the large bundle and the complexity of the owner test for arbitrary owners.

\paragraph{Acknowledgments.} The proofs, code and Lean formalization were developed with the assistance of AI coding agents (Claude Code); every claim here is machine-checked, computer-certified or refereed, as stated.
