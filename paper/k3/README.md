...
- `examples/`: verification of the examples of `long.tex` (see its Appendix D).
  - `trace_examples.py` recomputes every intermediate state of Examples 2 and 3 with the repository's literal transcription of the Lean program (`k3/k3algo.py`, `mirror`), compares the outputs with `mirror` and `fast`, checks them against the EFX₀ definition by brute force, and checks the side claims of the text (the EFX-but-not-EFX₀ allocations, the failing completions with owner r, the cases of the safety lemma (ledger L5), the repeated good in `HitSet`, ω = m − 2n + |NA|). Run from the repository root: `python3 paper/k3/examples/trace_examples.py` (output: `trace_output.txt`; exit status 0 iff every check passes).
