...
- `EFX/ThmF.lean`: Theorem F (`k4/c4min.md` §3.6, PR #41; ledger K4.C4MIN.F.LEAN): configurations with frozen agents
  ...
  frozen agents (`EFX.C4min.rigid_NA`, `EFX.C4min.frozen_of_min`), Lemma 1(a) with `C = ∅`
  (`EFX.C4min.removalOnly_of_cfg`), the free agents as an all-pairs allocation over `M ∖ 𝒩` (`EFX.C4min.isAPA_sub`,
...
| K4.C4MIN.F.LEAN | Theorem F: on every k = 4 core with a frozen-robust configuration at the fewest frozen agents, some min-frozen P ∈ 𝒫 has def(P) ≤ 0 and is completable (C₄ᵐⁱⁿ's conclusion, both forms); rigidity of the needed set at the fewest frozen agents; Lemma 1(a) with C = ∅ | ThmF : `EFX.C4min.theoremF_min`, `EFX.C4min.theoremF`, `EFX.C4min.c4minRO_of_frobust`, `EFX.C4min.rigid_NA`, `EFX.C4min.frozen_of_min`, `EFX.C4min.removalOnly_of_cfg`, `EFX.C4min.fmax_poolOpt`, `EFX.C4min.fmax_nRobust`, `EFX.C4min.frozen_cycle` |
