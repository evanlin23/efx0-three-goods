...
- `EFX/ThmZ.lean`: Theorem Z (`k4/c4min.md` §3, PR #41; ledger K4.C4MIN.Z.LEAN): all-pairs allocations
  ...
  Lemma Z2 (`EFX.C4min.threat_unique`, `threat_adm`, `threat_gain`), Lemmas P and R (`EFX.C4min.zvalid_of_zmax`), and
  Theorem Z: C₄ᵐⁱⁿ's conclusion in both forms on every k = 4 core whose fewest frozen agents is 0
  (`EFX.C4min.theoremZ_RO`, `EFX.C4min.theoremZ`; `EFX.C4min.theoremZ_min` with only the hypotheses used, n ≥ 1,
  3 ≤ |R_i| ≤ 4, every good relevant; `EFX.C4min.efx0_of_f0`, the EFX₀ allocation with at most one bundle of more than two
  goods). The choices where the prose leaves room are listed in the module doc.
