...
- `EFX/ThmZ.lean`: Theorem Z (`k4/c4min.md` §3, PR #41; ledger K4.C4MIN.Z.LEAN): all-pairs allocations
  ...
  Lemma Z1 (`EFX.C4min.exists_apa`, `EFX.C4min.exists_zmax`), Lemma Z2 (`EFX.C4min.threat_unique`, `threat_adm`,
  `threat_gain`), Lemmas P and R (`EFX.C4min.zvalid_of_zmax`), and `EFX.C4min.theoremZ`: C₄ᵐⁱⁿ's conclusion on every k = 4
  core whose fewest frozen agents is 0. The choices where the prose leaves room are listed in the module doc.
...
| K4.C4MIN.Z.LEAN | Theorem Z: on every k = 4 core whose fewest frozen agents is 0, some min-frozen P ∈ 𝒫 is completable (C₄ᵐⁱⁿ's conclusion); a pool-optimal all-pairs allocation with the most robust agents has a valid owner | ThmZ : `EFX.C4min.theoremZ`, `EFX.C4min.c4min_of_f0`, `EFX.C4min.zvalid_of_zmax`, `EFX.C4min.c4min_of_zvalid`, `EFX.C4min.exists_zmax`, `EFX.C4min.threat_unique`, `EFX.C4min.threat_gain` |
