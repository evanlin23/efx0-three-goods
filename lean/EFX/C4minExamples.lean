...
/-- `ω = |J| − S = 0 − 1 = −1`: removal-only completable without owner. -/
theorem removalOnly : RemovalOnly v ag gd b := by
  have : omegaP v ag gd b = -1 := by unfold omegaP; rw [capSum_eq]; decide
...
completable, even removal-only. -/
theorem c4min : IsCore4 v ag gd ∧ Strict v ag gd ∧ ∃ base, MinFrozen v ag gd base ∧ Completable v ag gd base ∧
    RemovalOnly v ag gd base ∧ nFrozen v ag gd base = 2 :=
  ⟨core, strict, b, minFrozen, completable, removalOnly, nFrozen_b⟩
